...
@Component
public class YxKcEnterpriseDaoImpl  extends BsBaseDaoTJdbc implements YxKcEnterpriseDao {
...
//            condition += BusinessUtil.getCommonCondition(user, null);
            ...
            String sql = "SELECT * FROM ( SELECT e.*,(select yy.yxcbs_uid from yx_yxcbs yy where yy.cbs_type='"+Constants.YXCBS_LX_KC+"'" +
    		"  and yy.company_uid=e.kc_enterprise_uid ) as yxcbs_uid FROM YX_KC_ENTERPRISE e ) t";
...
//            condition += BusinessUtil.getCommonCondition(user, null);
            ...
            String sql = "SELECT * FROM YX_KC_ENTERPRISE_HIS t";
            BaseResultSet bs = DBUtil.query(conn, sql, page);
...
//    	User user = ActionContext.getCurrentUserInThread();
        ...
        try {
            ...
            String sql = "SELECT * FROM (select s.* from yx_kc_enterprise_shjl s ) t";
            BaseResultSet bs = DBUtil.query(conn, sql, page);
    ...
    public String getDengluCode() throws Exception{
		...
		try {
			...
			String sql = "select  max(to_number(substr(t.denglu_code,6,9))) from yx_kc_enterprise t where t.denglu_code like 'WNDKC%'";
			...
			code = "WNDKC" + code;
			 ...
			 DaoException.handleMessageException("*********查询勘察设计企业登录代码出错!*********");
		} finally {
			DBUtil.closeConnetion(conn);
		}
		return code;
	...
	public JSONObject hisQyxxJSONObject(String uid) {
        ...
        try {
            String sql = "select * from yx_kc_enterprise_his t where t.kc_enterprise_uid ='"+uid+"' order by t.createddate desc ";
