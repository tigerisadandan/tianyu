package com.ccthanking.business.sgenter.manage;
...
public class SgEnterPriseLibraryManager implements Cache {
	private Hashtable usertable;
	static private SgEnterPriseLibraryManager instance;
	static private org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger("SgEnterPriseLibraryManager");

	private SgEnterPriseLibraryManager() throws Exception {
		init();
	}
		...
		try {
			...
			if (list != null) {
				...
				for (int i = 0; i < list.length; i++) {
                    ...
                    user.setPWD((Encipher.DecodePasswd(list[i][1])));
	...
	static synchronized public SgEnterPriseLibraryManager getInstance() throws Exception {
		if (instance == null) {
			instance = new SgEnterPriseLibraryManager();
