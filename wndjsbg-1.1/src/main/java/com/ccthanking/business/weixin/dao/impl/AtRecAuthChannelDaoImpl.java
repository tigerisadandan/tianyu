...
@Component
public class AtRecAuthChannelDaoImpl  extends BsBaseDaoTJdbc implements AtRecAuthChannelDao {
...
//            condition += BusinessUtil.getCommonCondition(user, null);
            ...
            String sql = "select * from (SELECT vc.fr as fr,vc.pfr as pfr,vc.account as account, vc.name as name,vc.user_sn as usersn, "+
            	" ac.channel_uid as channel_uid,ac.recid as recid,ac.rec_auth_channel_uid, "+
            	" (select wc.channel_name from WX_CHANNEL wc where wc.channel_uid=ac.channel_uid) as channel_name  "+
            	" FROM AT_REC_AUTH_CHANNEL ac "+
            	" left join v_at_auth_channel vc on vc.account=ac.recid) t ";
    ...
    public void awardToUsers(String channelid, String[] userids,
		...
		try {
			...
			String deleteSql = "DELETE FROM AT_REC_AUTH_CHANNEL F WHERE F.channel_uid ='" + channelid + "'";
			boolean bDel = DBUtil.exec(conn, deleteSql);

			if (bDel) {
				for (int i = 0; i < userids.length; i++) {
					String uuid=new RandomGUID().toString();
					String addSql = "INSERT INTO AT_REC_AUTH_CHANNEL(REC_AUTH_CHANNEL_UID,CHANNEL_UID,RECID,CREATED_UID,CREATED_NAME,CREATED_DATE)" +
