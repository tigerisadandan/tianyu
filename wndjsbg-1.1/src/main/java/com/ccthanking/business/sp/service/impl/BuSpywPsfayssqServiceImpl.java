...
@Service
public class BuSpywPsfayssqServiceImpl extends Base1ServiceImpl<BuSpywPsfayssqVO, String> implements BuSpywPsfayssqService {

	private static Logger logger = LoggerFactory.getLogger(BuSpywPsfayssqServiceImpl.class);
	
	// 业务类型
    private String ywlx = YwlxManager.BU_SPYW_PSFAYSSQ;
    
    private BuSpywPsfayssqDao buSpywPsfayssqDao;
    @Autowired
    private BuSpLzhfService buSpLzhfService;
    @Autowired
	@Qualifier("buSpywPsfayssqDaoImpl")
	public void setbuSpywPsfayssqDao(BuSpywPsfayssqDao buSpywPsfayssqDao) {
		this.buSpywPsfayssqDao = buSpywPsfayssqDao;
		super.setBsBaseDaoTJdbc((BsBaseDaoTJdbc) buSpywPsfayssqDao);
	}
    // @Override
    public String download(String id) throws Exception {
        
    	
    	BuSpywPsfayssqVO tmp =this.findById(id);
    	String templatePath=Constants.getString(Constants.PATH_TEMPLATE_WORD, "");
    	String fileName_pre = Constants.getString("template_word_js_14", "无锡新区城市排水方案预审申请表");
    	String templateName=fileName_pre;
    	String fileName="无锡新区城市排水方案预审申请表"+id;//+tmp.getPsfayssq_uid();
    	...
    	BuSpywPsfayssqVO vo=new BuSpywPsfayssqVO();
 		...
 		try {
 			...
 			LogManager.writeUserLog(null, ywlx, Globals.OPERATION_TYPE_DELETE, LogManager.RESULT_FAILURE,
				user.getName() + "排水方案预审申请—数据查询失败", user, "", "");
 			SystemException.handleMessageException("排水方案预审申请—数据查询失败,请联系相关人员处理");
 		} finally {
 		}
 			return domresult;
    ...
    public String insert(String json) throws Exception {
    	...
    	BuSpywPsfayssqVO vo=new BuSpywPsfayssqVO();
 		String domresult = null;
 		try {
 			domresult = buSpywPsfayssqDao.insert(json);
 			LogManager.writeUserLog(vo.getPsfayssq_uid(), ywlx, Globals.OPERATION_TYPE_DELETE, LogManager.RESULT_SUCCESS,
				user.getName() + "排水方案预审申请—数据新增成功", user, "", "");

 		} catch (DaoException e) {
 			logger.error("表单信息{}", e.getMessage());
 			LogManager.writeUserLog(vo.getPsfayssq_uid(), ywlx, Globals.OPERATION_TYPE_DELETE, LogManager.RESULT_FAILURE,
				user.getName() + "排水方案预审申请—数据新增失败", user, "", "");
 			SystemException.handleMessageException("排水方案预审申请—数据新增失败,请联系相关人员处理");
 		} finally {
 		}
 			return domresult;
    ...
    public String update(String json) throws Exception {
    	...
    	BuSpywPsfayssqVO vo=new BuSpywPsfayssqVO();
 		String domresult = null;
 		try {
 			domresult = buSpywPsfayssqDao.update(json);
 			LogManager.writeUserLog(vo.getPsfayssq_uid(), ywlx, Globals.OPERATION_TYPE_DELETE, LogManager.RESULT_SUCCESS,
				user.getName() + "排水方案预审申请—数据更改成功", user, "", "");

 		} catch (DaoException e) {
 			logger.error("表单信息{}", e.getMessage());
 			LogManager.writeUserLog(vo.getPsfayssq_uid(), ywlx, Globals.OPERATION_TYPE_DELETE, LogManager.RESULT_FAILURE,
				user.getName() + "排水方案预审申请—数据更改失败", user, "", "");
 			SystemException.handleMessageException("排水方案预审申请—数据更改失败,请联系相关人员处理");
 		} finally {
 		}
 			return domresult;
    ...
    public String delete(String json) throws Exception {
    	...
    	BuSpywPsfayssqVO vo=new BuSpywPsfayssqVO();
 		...
 		try {
 			domresult = null;
 			LogManager.writeUserLog(vo.getPsfayssq_uid(), ywlx, Globals.OPERATION_TYPE_DELETE, LogManager.RESULT_SUCCESS,
				user.getName() + "排水方案预审申请—数据删除成功", user, "", "");

 		} catch (DaoException e) {
 			logger.error("表单信息{}", e.getMessage());
 			LogManager.writeUserLog(vo.getPsfayssq_uid(), ywlx, Globals.OPERATION_TYPE_DELETE, LogManager.RESULT_FAILURE,
				user.getName() + "排水方案预审申请—数据删除失败", user, "", "");
 			SystemException.handleMessageException("排水方案预审申请—数据删除失败,请联系相关人员处理");
 		} finally {
 		}
 			return domresult;
...
//		obj.LINGJIAN_PHONE=ljrdh;
        try {
            ...
            String id= buSpywPsfayssqDao.getIdByYwlzuid(YWLZ_UID);
            BuSpywPsfayssqVO mapFtl=new BuSpywPsfayssqVO();
