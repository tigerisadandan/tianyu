...
@Component
public class BuSpywHjyxdjgzcns2SpDaoImpl  extends BsBaseDaoTJdbc implements BuSpywHjyxdjgzcns2SpDao {
	private static Logger logger = LoggerFactory.getLogger(BuSpywHjyxdjgzcns2SpDaoImpl.class);
	private  String SQL_QUERY="select hjyxdjgzcns_uid, ywlz_uid, xmmc, jsdw, tbrq_date, frdb," +
			" lxr, lxdh, cz, yzbm, txl, jsdd, zdmj, rhmj, ztz, hbtz, yqtcrq, yjgzr, syt, ydxz, jsxmnr," +
			" jsgm, scgm, qtqk, nkgrq, jhjsq, cqcs, tqsm from bu_spyw_hjyxdjgzcns2_sp";
	 ...
	 public String queryCondition(String json){
	        ...
	        try {
	            ...
	            logger.error("环境影响登记告知承诺书审批--附件三查询失败!");
	 ...
	 //查询附件的方法
	    public String queryReadFile(String id) throws Exception {
	        ...
	        String ListMX="select file_save_name,at_fileupload_uid from at_fileupload where target_uid="+id+" and file_type=39 and business_sub_type='BU_SPYW_HJYXDJGZCNS2_SP'";
	    	String filePath="";
	    	String fileName="";
	    	String uid="";
	    	try {
		...
		public String insert(String json,Map files) throws Exception {
	        ...
	        vo.setHjyxdjgzcns_uid(DBUtil.getSequenceValue("HJYXDJGZCNS2_SP_UID"));
	        ...
	        conn.commit();
	        	...
	        	DBUtil.rollbackConnetion(conn);
	        	logger.error("环境影响登记告知承诺书审批--附件三新增失败!");
	            e.printStackTrace(System.out);
	        } finally {
	            DBUtil.closeConnetion(conn);
	        }
		...
		public String update(String json,Map files) throws Exception {
	        ...
	        try {
	            ...
	            DBUtil.rollbackConnetion(conn);
	            logger.error("环境影响登记告知承诺书审批--附件三更改失败!");
	            e.printStackTrace(System.out);
	        } finally {
	            DBUtil.closeConnetion(conn);
	        }
