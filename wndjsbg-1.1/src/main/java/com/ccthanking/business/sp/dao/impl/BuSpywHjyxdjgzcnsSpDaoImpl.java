...
@Component
public class BuSpywHjyxdjgzcnsSpDaoImpl  extends BsBaseDaoTJdbc implements BuSpywHjyxdjgzcnsSpDao {
	private static Logger logger = LoggerFactory.getLogger(BuSpywHjyxdjgzcnsSpDaoImpl.class);
	private static String SQL_QUERY="select hjyxdjgzcns_uid, ywlz_uid, xmmc, jsdw, tbrq_date," +
			...
			" yjgzr, syt, s_xhl, ry_xhl, d_xhl, rq_xhl, rm_xhl, qt, qtqksm, scgylcjs, ncydwrfzcs from bu_spyw_hjyxdjgzcns_sp";
	...
	public String queryCondition(String json) throws Exception {
        ...
        try {
            ...
            logger.error("环境影响登记告知承诺书审批信息查询失败!");
    ...
    public String queryReadFile(String id) throws Exception {
        ...
        String ListMX="select file_save_name,at_fileupload_uid from at_fileupload where target_uid="+id+" and file_type=39 and business_sub_type='BU_SPYW_HJYXDJGZCNS_SP'";
    	String filePath="";
    	String fileName="";
    	String uid="";
    	try {
	...
	public String insert(String json,Map files) throws Exception {
        ...
        BuSpywHjyxdjgzcnsSpVO vo = new BuSpywHjyxdjgzcnsSpVO();
        BuSpywHjyxdjgzcnsNrgmSpVO nvo=new BuSpywHjyxdjgzcnsNrgmSpVO();
        BuSpywHjyxdjgzcnsZyssSpVO svo=new BuSpywHjyxdjgzcnsZyssSpVO();
        try{
        conn.setAutoCommit(false);
        ...
        vo.setHjyxdjgzcns_uid(DBUtil.getSequenceValue("HJYXDJGZCNS_SP_UID"));
        ...
        if(sqmxArray!=null){
        	for (int i = 0; i < sqmxArray.size(); i++) {//判断添加到子表的记录的数量
				...
				//获取索引
		        svo.setHjyxdjgzcnszyss_uid(DBUtil.getSequenceValue("HJYXDJGZCNS_ZYSS_SP_UID"));
        ...
        if(nrgmArray!=null){
        	for (int i = 0; i < nrgmArray.size(); i++) {//判断添加到子表的记录的数量
				...
				//获取索引
		        nvo.setHjyxdjgzcnsnrgm_uid(DBUtil.getSequenceValue("HJYXDJGZCNS_NRGM_SP_UID"));
        ...
        conn.commit();
        	...
        	DBUtil.rollbackConnetion(conn);
            e.printStackTrace(System.out);
            logger.error("环境影响登记告知承诺书审批信息表新增失败!");
        } finally {
            DBUtil.closeConnetion(conn);
        }
		return resultVO;
		 ...
		 Connection conn = DBUtil.getConnection();
	        ...
	        try {
	            ...
	            if(!"".equals(MXList)){
	        		...
	        		//清楚多余页面被删掉的内容规模信息
	        		String clear_nrgm_SQL = "delete from bu_spyw_hjyxdjgzcns_nrgm_sp t where t.hjyxdjgzcns_uid = "+vo.getHjyxdjgzcns_uid()+" and t.hjyxdjgzcnsnrgm_uid not in ("+MXList+")";
	        		DBUtil.exec(conn, clear_nrgm_SQL);

	        	}else{
	        		//删除所有内容规模信息
	            	String clear_nrgm_SQL = "delete from bu_spyw_hjyxdjgzcns_nrgm_sp t where t.hjyxdjgzcns_uid = "+ vo.getHjyxdjgzcns_uid();
	        		DBUtil.exec(conn, clear_nrgm_SQL);
	            ...
	            if(!"".equals(zyssList)){
	        		...
	        		//清楚多余页面被删掉的内容规模信息
	        		String clear_zyss_SQL = "delete from bu_spyw_hjyxdjgzcns_zyss_sp t where t.hjyxdjgzcns_uid = "+vo.getHjyxdjgzcns_uid()+" and t.hjyxdjgzcnszyss_uid not in ("+zyssList+")";
	        		DBUtil.exec(conn, clear_zyss_SQL);

	        	}else{
	        		//删除所有内容规模信息
	            	String clear_zyss_SQL = "delete from bu_spyw_hjyxdjgzcns_zyss_sp t where t.hjyxdjgzcns_uid = "+ vo.getHjyxdjgzcns_uid();
	        		DBUtil.exec(conn, clear_zyss_SQL);
	            ...
	            DBUtil.rollbackConnetion(conn);
	            e.printStackTrace(System.out);
	            logger.error("环境影响登记告知承诺书审批信息表更改失败!");
	        } finally {
	            DBUtil.closeConnetion(conn);
	        }
			return null;
	 ...
	 * @throws Exception
	 */
	public String getMX( Connection conn , JSONObject obj, BuSpywHjyxdjgzcnsSpVO vo) throws Exception{
		JSONArray MXArray = null;
		String emptyMX = "";
		BuSpywHjyxdjgzcnsNrgmSpVO MXVO = null;
        User user = ActionContext.getCurrentUserInThread();
        ...
        if(MXArray!=null){
        	for (int i = 0; i < MXArray.size(); i++) {
				...
				//获取索引
				MXVO = new BuSpywHjyxdjgzcnsNrgmSpVO();
	            String zeng_uid = obj.getJSONArray("HJYXDJGZCNSNRGM_UID").getString(i);
	            boolean flag = true;
	            if(zeng_uid==null||"".equals(zeng_uid)){
	            	//若不存在uid则执行新增,反之进行修改
	            	flag = false;
	            	MXVO.setHjyxdjgzcnsnrgm_uid(DBUtil.getSequenceValue("HJYXDJGZCNS_NRGM_SP_UID"));
	            }else{
	...
	public String getZYSS( Connection conn , JSONObject obj, BuSpywHjyxdjgzcnsSpVO vo) throws Exception{
		JSONArray MXArray = null;
		String emptyMX = "";
		BuSpywHjyxdjgzcnsZyssSpVO MXVO = null;
        User user = ActionContext.getCurrentUserInThread();
        ...
        if(MXArray!=null){
        	for (int i = 0; i < MXArray.size(); i++) {
				...
				//获取索引
				MXVO = new BuSpywHjyxdjgzcnsZyssSpVO();
	            String zeng_uid = obj.getJSONArray("HJYXDJGZCNSZYSS_UID").getString(i);
	            boolean flag = true;
	            if(zeng_uid==null||"".equals(zeng_uid)){
	            	//若不存在uid则执行新增,反之进行修改
	            	flag = false;
	            	MXVO.setHjyxdjgzcnszyss_uid(DBUtil.getSequenceValue("HJYXDJGZCNS_ZYSS_SP_UID"));
	            }else{
