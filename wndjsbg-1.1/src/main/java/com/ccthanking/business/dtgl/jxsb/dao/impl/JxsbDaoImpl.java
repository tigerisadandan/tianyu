...
@Component
public class JxsbDaoImpl  extends BsBaseDaoTJdbc implements JxsbDao {
    ...
    public String queryCondition(String json, JxsbVO vo, Map map){
        ...
        try {
            ...
            String sql = "SELECT * FROM ( SELECT T.*," +
            		               "(SELECT U.USER_NAME FROM USERS U WHERE U.USERS_UID=T.SHENHE_REN ) AS SHENHENAME," +
            		               "(SELECT M.AZ_COMPANY_UID FROM AZ_COMPANY M WHERE M.AZ_COMPANY_UID=T.CREATED_UID) AS AZ_COMPANY_UID," +
            		               "(SELECT M.COMPANY_NAME FROM AZ_COMPANY M WHERE M.AZ_COMPANY_UID=T.CREATED_UID) AS COMPANY_NAME " +
            		     "FROM JXSB T ) "; 
            ...
            bs.setFieldDic("SY_STATUS", "AZ_JXSB_SY_STATUS");
