...
public class BgMenuServiceImpl implements BgMenuService {
	...
	private static Logger logger = LoggerFactory.getLogger(BgMenuServiceImpl.class.getName());
	...
	public String loadAllMenu(String type,String id) {
		Connection conn = DBUtil.getConnection();
		String queryDictSql = "";
		/*
		if(account != null && !account.equals("null") && !account.equals("") ){
			//查出用户拥有的权限
			queryDictSql = "select UR.ROLE_CODE ROLE_CODE,T.NAME,T.TITLE,T.PARENT,T.ORDERNO,T.TARGET,T.LOCATION,T.CHIEF,T.LEVELNO,T.CODE " +
					" from (SELECT * FROM FS_EAP_MENU WHERE SFYX = '1' and APP_NAME = 'wndjsbg' ) T " +
					" left join FS_USER_ROLE UR on T.CODE = UR.ROLE_CODE and UR.USERS_UID = '"+account+"' order by T.ORDERNO ";
		}
		if(roleId!=null && !roleId.equals("null") &&!roleId.equals("")){
			//查出角色拥有的权限
			queryDictSql = "SELECT RM.MENU_NAME MENU_NAME, T.NAME, T.TITLE, T.PARENT, "
					+ " T.ORDERNO, T.TARGET, T.LOCATION, T.CHIEF, T.LEVELNO "
					+ " FROM (SELECT * FROM FS_EAP_MENU WHERE SFYX='1' and APP_NAME = 'wndjsbg' ) T LEFT JOIN FS_ORG_ROLE_MENU_MAP RM "
					+ " ON T.NAME = RM.MENU_NAME AND RM.ROLE_ID = '" + roleId + "' ORDER BY T.ORDERNO";
		}
		*/
		if (type.equals("user")) {
			//查出用户拥有的权限
			queryDictSql = "select UR.ROLE_CODE,T.NAME,T.CODE,T.TITLE,T.PARENT,T.ORDERNO,T.TARGET,T.LOCATION,T.CHIEF,T.LEVELNO,T.CODE " +
					" from (SELECT * FROM FS_EAP_MENU WHERE SFYX = '1' and APP_NAME in ('wndjsbg','wndjsbggdzxt') ) T " +
					" left join FS_USER_ROLE UR on T.CODE = UR.ROLE_CODE and UR.USERS_UID = "+id+" order by T.ORDERNO ";
		}else if(type.equals("role")){
			//查出角色拥有的权限
			queryDictSql = "SELECT RM.MENU_NAME MENU_NAME,T.CODE,T.NAME,T.TITLE, T.PARENT, "
					+ " T.ORDERNO, T.TARGET, T.LOCATION, T.CHIEF, T.LEVELNO "
					+ " FROM (SELECT * FROM FS_EAP_MENU WHERE SFYX='1' and APP_NAME in ('wndjsbg','wndjsbggdzxt') ) T LEFT JOIN FS_ORG_ROLE_MENU_MAP RM "
					+ " ON T.NAME = RM.MENU_NAME AND RM.ROLE_ID = '" + id + "' ORDER BY T.ORDERNO";
		}
		JSONArray jsonArr = new JSONArray();
		try {
			...
			if(type.equals("user")){
				...
				for (int i = 0; i < rsList.size(); i++) {
					...
					JSONObject json = new JSONObject();
					json.put("id", rsMap.get("NAME"));
					if (rsMap.get("ROLE_CODE").equals(rsMap.get("CODE"))) {
						json.put("checked", "true");
						json.put("open", "true");
					}
					json.put("parentId", rsMap.get("PARENT"));
					json.put("name", rsMap.get("TITLE"));
					json.put("code", rsMap.get("CODE"));
					jsonArr.add(json);
				}
			}else if(type.equals("role")){
				//角色
				for (int i = 0; i < rsList.size(); i++) {
					...
					JSONObject json = new JSONObject();
					json.put("id", rsMap.get("NAME"));
					if (rsMap.get("MENU_NAME").equals(rsMap.get("NAME"))) {
						json.put("checked", "true");
						json.put("open", "true");
					}
					json.put("parentId", rsMap.get("PARENT"));
			...
			for (int i = 0; i < rsList.size(); i++) {
				...
				if(account != null && !account.equals("null") && !account.equals("")){
					//查出用户拥有的权限
					json.put("code", rsMap.get("ROLE_CODE"));
					if(rsMap.get("ROLE_CODE").equals(rsMap.get("CODE"))){
						json.put("checked",
						 "true");
						json.put("open", "true");
	...
	public void awardMenuToUser(String userId, String[] menuName,String[] codes, User user) throws Exception {
		...
		try {
			conn = DBUtil.getConnection();
			conn.setAutoCommit(false);
			String deleteMenuByUserSql = "DELETE FS_USER_ROLE T WHERE T.USERS_UID = "+userId;
			boolean bDel = DBUtil.exec(conn, deleteMenuByUserSql);
			if (bDel) {
				for (int i = 0; i < menuName.length; i++) {
					String addMenuToUserSql = "INSERT INTO FS_USER_ROLE VALUES ('"+codes[i]+"','"+menuName[i]+"',"+userId+") ";
