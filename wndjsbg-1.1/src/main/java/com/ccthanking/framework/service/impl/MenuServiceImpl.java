...
public class MenuServiceImpl implements MenuService {
	...
	private static Logger logger = LoggerFactory
			.getLogger(MenuServiceImpl.class.getName());

	// 本系统的 所有菜单sql
	private static final String SQL_APPMENU = "select name,code, location from fs_eap_menu where app_name='"
			+ Constants.APP_NAME
			+ "' and sfyx=1 and location is not null and location !='#'";
	...
	public String getAllMenu() {
		...
		/*
		String queryDictSql2 = "SELECT NAME, TITLE, PARENT, ORDERNO, TARGET, LOCATION, "
				+ "LEVELNO, IMAGE, ALTIMAGE, APP_NAME, LAYERSNO, MEMO, LRR, LRSJ, LRBM, "
				+ "LRBMMC, GXR, GXSJ, GXBM, GXBMMC, SJMJ, SFYX FROM FS_EAP_MENU " + "WHERE quanxian_package.get_bg_user_quanxian(code, 用户UID)=1 and menu_y='Y' and SFYX='1' ORDER BY ORDERNO";
		*/
		...
		try {
			List<Map<String, String>> rsList = DBUtil.queryReturnList(conn,
					queryDictSql);
	...
	public String executeMenu(String json, User user, String operatorSign)
		...
		try {
			...
			case 1:
				...
				int maxOrderNo = Integer.parseInt(orderNo == null ? "1"
						: orderNo[0][0]) + 1;
				...
				com.ccthanking.framework.coreapp.orgmanage.MenuManager
						.getInstance().synchronize(menuVo.getName(),
								CacheManager.ADD);
				break;
			case 2:
				msg = "修改";
	...
	public String queryUnique(String menuName, User user) {
		String sql = "SELECT COUNT(1) CNT FROM FS_EAP_MENU WHERE NAME = '"
				+ menuName + "'";
	...
	public String loadAllMenu(String roleId) {
		...
		String queryDictSql = "SELECT RM.MENU_NAME MENU_NAME, T.NAME, T.TITLE, T.PARENT, "
				...
				+ " ON T.NAME = RM.MENU_NAME AND RM.ROLE_ID = '"
				+ roleId
				+ "' ORDER BY T.ORDERNO";
		JSONArray jsonArr = new JSONArray();
		try {
			...
			String resourceSql = "select "
					...
					+ " ON t.FS_PAGE_RESOURCE_ID = f.MENU_NAME AND f.ROLE_ID = '"
					+ roleId + "'";
			List<Map<String, String>> resourceList = DBUtil.queryReturnList(
					conn, resourceSql);
			for (int i = 0; i < resourceList.size(); i++) {
				...
				if (resourceMap.get("FS_PAGE_RESOURCE_ID").equals(
						resourceMap.get("MENU_NAME"))) {
					json.put("checked", "true");
					json.put("open", "true");
				}
	...
	public void awardMenuToRole(String roleId, String[] menuName, User user)
			throws Exception {
		Connection conn = null;
		try {
			conn = DBUtil.getConnection();
			conn.setAutoCommit(false);
			String deleteMenuByRoleSql = "DELETE FROM FS_ORG_ROLE_MENU_MAP WHERE ROLE_ID='"
					+ roleId + "'";
	...
	public List<Map<String, String>> getAll() {
		...
		Connection conn = DBUtil.getConnection();
		try {
			List<Map<String, String>> lMap = DBUtil.queryReturnList(conn,
					SQL_APPMENU);
			return lMap;
		} catch (Exception e) {
		} finally {
	...
	public String[][] loadRoleByMid(String menuid) {
		...
		try {
			return DBUtil
					.querySql(
							conn,
							"select menu_name, role_id from fs_org_role_menu_map where menu_name=?",
							new Object[] { menuid });
		} catch (Exception e) {
		...
		if (menus != null && menus.length > 0) {
			for (int i = 0; i < menus.length; i++) {
				...
				if (!"#".equals(menus[i].getLocation())) {
					menuTreeHtml += "<a href=\"#\"onclick=\"menutree_click('"
							+ menus[i].getName() + "','"
							+ menus[i].getLocation() + "','"
							+ menus[i].getTitle() + "','"
							+ menus[i].getTarget() + "')\">";
				...
				menuTreeHtml += " <i class=\"" + icss
						+ " icon-white\"></i>  <span class=\"title\">"
				...
				if (menus_two != null && menus_two.length > 0) {
					...
					for (int j = 0; j < menus_two.length; j++) {
						...
						menuTreeHtml += "<li> <a href=\"#\"onclick=\"menutree_click('"
								+ menus_two[j].getName()
								+ "','"
								...
								+ "')\">"
								+ menus_two[j].getTitle() + "</a>\r\n";
						MenuVo[] menus_three = getAllowedMenus(user,
								menus_two[j].getName());
						if (menus_three != null && menus_three.length > 0) {
							...
							for (int k = 0; k < menus_three.length; k++) {
								menuTreeHtml += " <li><a href=\"javascript:void(0);\" onclick=\"menutree_click('"
										+ menus_three[k].getName()
										+ "','"
										+ menus_three[k].getLocation()
										+ "','"
										+ menus_three[k].getTitle()
										+ "','"
										+ menus_three[k].getTarget()
										+ "')\">"
										+ menus_three[k].getTitle()
										+ "</a></i>\r\n";
							}
						...
						} else if (menus_three == null
								|| menus_three.length == 0) {
	...
	protected MenuVo[] getAllowedMenus(User user, String parent)
			throws Exception {
		// 超级用户，看见系统中所有菜单
	...
	public String[][] loadUserByMid(String menuid) {
		...
		try {
			return DBUtil
					.querySql(
							conn,
							"select role_code,users_uid from fs_user_role where role_code = ?",
							new Object[] { menuid });
		} catch (Exception e) {
			DaoException.handleException("***根据菜单ID查询角色 出现错误！***");
		} finally {
