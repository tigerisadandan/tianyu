...
@Service
public class UserServiceImpl extends Base1ServiceImpl<UserVO, String> implements UserServiceCust {
	...
	public String queryUser(String json) throws Exception {
		...
		try {
			...
			String sql = "SELECT ACCOUNT, PASSWORD, NAME, SEX, DEPARTMENT, PARENT, PERSON_KIND, "
					...
					+ "MAILNAME, MAILPSW, USERTEMPLATE, JWQ, ZRQ, SJHM,SORT FROM v_ORG_PERSON";
	...
	public String executeUser(String json, User user, String id, String update) throws Exception {
		...
		try {
			...
			case 1:
				...
				//usersVo.setYwid(jsonObj.getString("YWID"));
				usersVo.setUsers_uid(jsonObj.getString("USERS_UID"));
				//fvo.setYwid(usersVo.get);
				...
				com.ccthanking.framework.coreapp.orgmanage.UserManager.getInstance().synchronize(
						jsonObj.getString("USERS_UID"), CacheManager.ADD);
				break;
			case 2:
				msg = "修改";
				usersVo.setValueFromJson(jsonObj);
				...
				com.ccthanking.framework.coreapp.orgmanage.UserManager.getInstance().synchronize(
						jsonObj.getString("USERS_UID"), CacheManager.UPDATE);
				break;
			case 3:
				msg = "刪除";
				// 用标识来表示信息被删除
				...
				com.ccthanking.framework.coreapp.orgmanage.UserManager.getInstance().synchronize(
						jsonObj.getString("USERS_UID"), CacheManager.DELETE);
				break;
	...
	public void awardRoleToUser(String account, String[] roleNameAndId, User user) throws Exception {
		...
		try {
			...
			LogManager.writeUserLog(user.getAccount(), YwlxManager.SYSTEM_USER, Globals.OPERATION_TYPE_INSERT,
					LogManager.RESULT_SUCCESS, user.getDepartment() + " " + user.getName() + "为用户分配角色成功",
					user, "", "");
			...
			e.printStackTrace();
			LogManager.writeUserLog(user.getAccount(), YwlxManager.SYSTEM_USER, Globals.OPERATION_TYPE_INSERT,
					LogManager.RESULT_SUCCESS, user.getDepartment() + " " + user.getName() + "为用户分配角色失败",
					user, "", "");
		} finally {
			DBUtil.closeConnetion(conn);
		}
	...
	public void awardRoleToPerson(String roleid, String rolename, String[] personNameAndId, User user) throws Exception {
		...
		try {
			...
			conn.commit();
			LogManager.writeUserLog(user.getAccount(), YwlxManager.SYSTEM_USER, Globals.OPERATION_TYPE_INSERT,
					LogManager.RESULT_SUCCESS, user.getOrgDept().getDeptName() + " " + user.getName() + "将角色授予用户成功",
					user, "", "");
		} catch (Exception e) {
	...
	public String resetPw(String account) throws Exception {
		...
		try {
			...
			if (res != null) {
				...
				// 默认密码
				String default_pwd = Constants.getString("default_pwd", "111111");
				// String newPass =
				// Encipher.EncodePasswd(default_pwd.toUpperCase());
				String newPass = default_pwd;
				String newPass_md5 = default_pwd.toUpperCase();
				if (Constants.ENCODER_TYPE.equals("MD5")) {// md5加密
				...
				TreeNode appNode = Dics.getDicByName("APP_NAME");
				if (appNode != null) {
					TreeNode[] childs = appNode.getChilds();
					for (TreeNode treeNode : childs) {
						String appname = treeNode.getDicCode();
						if (StringUtils.isNotBlank(res[0][1]) && res[0][1].equals(appname)) {
							// 获取更新sql语句
							SysParaConfigureVO sysVO = ParaManager.getInstance().getSysParameter(appname);
							if (sysVO != null) {
								String para1 = sysVO.getSysParaConfigureParavalue1();
								String para2 = sysVO.getSysParaConfigureParavalue2();
								if (StringUtils.isNotBlank(para2)) {
									para1 = para1.replaceAll(para2, newPass_md5);
								}
								String para3 = sysVO.getSysParaConfigureParavalue3();
								if (StringUtils.isNotBlank(para3)) {
									para1 = para1.replaceAll(para3, account.toUpperCase());
								}
								String para4 = sysVO.getSysParaConfigureParavalue4();
								if (StringUtils.isNotBlank(para4)) {
									para1 = para1.replaceAll(para4, newPass);
								}

								DBUtil.exec(conn, para1);
								conn.commit();
							}
						}
					}
				...
				// if (res[0][1].equals("1")) {
				// // 从表fs_org_person来
				// sql = "update fs_org_person t set t.password='" + newPass_md5
				// + "' where  upper(t.account)='"
				// + account.toUpperCase() + "'";
				// } else if (res[0][1].equals("2")) {
				// sql = "update sg_enterprise_library t set t.pwd='" +
				// newPass_md5 + "', MIMA='" + newPass
				// + "' where  upper(t.denglu_code)='" + account.toUpperCase() +
				// "' and status='1'";
				// }
