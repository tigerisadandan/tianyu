...
import com.ccthanking.framework.service.BgRoleService;
...
public class BgRoleServiceImpl implements BgRoleService {
	 ...
	 * 
	 * @see
	 * com.ccthanking.framework.service.impl.RoleService#queryRole(java.lang
	 * .String, com.ccthanking.framework.common.User)
	 */
	 ...
	 * 
	 * @see
	 * com.ccthanking.framework.service.impl.RoleService#executeRole(java.lang
	 * .String, com.ccthanking.framework.common.User, java.lang.String)
	 */
	public String executeRole(String json, User user, String operatorSign) throws Exception {
		Connection conn = DBUtil.getConnection();
		String msg = null;
		...
		try {
			...
			case 1:
				...
				//添加时写入公共字段
				roleVo.setInternal("GXR",user.getAccount());
				roleVo.setInternal("GXSJ",Pub.getCurrentDate());
				//roleVo.setInternal("GXBM",user.getDepartment());
				roleVo.setInternal("LRR",user.getAccount());
				roleVo.setInternal("LRSJ",roleVo.getInternal("GXSJ"));
				//roleVo.setInternal("LRBM",user.getDepartment());
				//roleVo.setInternal("LRBMMC",user.getDepartment());
				roleVo.setInternal("SFYX","1");
				BaseDAO.insert(conn, roleVo);
				resultVo = roleVo.getRowJson();
				conn.commit();

				com.ccthanking.framework.coreapp.orgmanage.OrgRoleManager.getInstance().synchronize(
						jsonObj.getString("ROLE_ID"), CacheManager.ADD);
				break;
			...
			case 2:
				...
				com.ccthanking.framework.coreapp.orgmanage.OrgRoleManager.getInstance().synchronize(
						jsonObj.getString("ROLE_ID"), CacheManager.UPDATE);
				break;
				...
				com.ccthanking.framework.coreapp.orgmanage.OrgRoleManager.getInstance().synchronize(
						jsonObj.getString("ROLE_ID"), CacheManager.DELETE);
				break;
				 */
			default:
				break;
			...
			LogManager.writeUserLog(user.getAccount(), YwlxManager.SYSTEM_ROLE, Globals.OPERATION_TYPE_INSERT,
					LogManager.RESULT_SUCCESS, user.getDepartment() + " " + user.getName() + msg + "角色信息成功",
					user, "", "");
		} catch (Exception e) {
			DBUtil.rollbackConnetion(conn);
			e.printStackTrace(System.out);
			LogManager.writeUserLog(user.getAccount(), YwlxManager.SYSTEM_ROLE, Globals.OPERATION_TYPE_INSERT,
					LogManager.RESULT_FAILURE, user.getDepartment() + " " + user.getName() + msg + "角色信息失败",
					user, "", "");
		} finally {
			DBUtil.closeConnetion(conn);
		}
	...
	public String queryRoleTree(HttpServletRequest request, String jsonString) {
		...
		try {
			...
			// String arrs[] = s.split(",");
			// for(String arr:arrs){
			// String checkedParentSql =
			// "select FS_ORG_ROLE_TREE from FS_ORG_ROLE_TREE where account='"+arr+"'";
	...
	public String queryRoleByTreeNode(HttpServletRequest request, String json) throws SQLException {
		...
		try {
			...
			String sql = "SELECT ROLE_ID, NAME, S_MEMO, LEVEL_NAME, DEPTID, YWLX, SJBH, BZ, "
					...
					+ " FROM FS_ORG_ROLE R," + " FS_ORG_ROLE_TREE L ";
			BaseResultSet bs = DBUtil.query(conn, sql, page);
			bs.setFieldOrgDept("DEPTID");
			bs.setFieldUserID("GXR");
			bs.setFieldDic("ROLETYPE", "JSLB");
	...
	public String doMoveInTreeNode(HttpServletRequest request, String json) throws Exception {
		...
		try {
			...
			if (IDArr != null && IDArr.length != 0) {
				...
				// 获取同级别节点中的最大排序号
				String sqlLevel = "select NODELEVEL from FS_ORG_ROLE_TREE where FS_ORG_ROLE_TREE_ID='"
						+ vo.getParentid() + "'";
			...
			sjbh = event.getSjbh();
			LogManager.writeUserLog(sjbh, YwlxManager.SYSTEM_ROLE_TREE, Globals.OPERATION_TYPE_INSERT,
					LogManager.RESULT_SUCCESS, user.getOrgDept().getDeptName() + " " + user.getName() + "加入角色类别成功",
					user, "", "");
			conn.commit();
			...
			LogManager.writeUserLog(sjbh, YwlxManager.SYSTEM_ROLE_TREE, Globals.OPERATION_TYPE_INSERT,
					LogManager.RESULT_FAILURE, user.getOrgDept().getDeptName() + " " + user.getName() + "加入角色类别失败",
					user, "", "");
			e.printStackTrace(System.out);
			DBUtil.rollbackConnetion(conn);
			throw e;
	...
	public String doMoveOutTreeNode(HttpServletRequest request, String json) throws Exception {
		...
		try {
			...
			LogManager.writeUserLog(sjbh, YwlxManager.SYSTEM_ROLE_TREE, Globals.OPERATION_TYPE_UPDATE,
					LogManager.RESULT_SUCCESS, user.getOrgDept().getDeptName() + " " + user.getName() + "加入角色类别成功",
					user, "", "");
			conn.commit();
		} catch (Exception e) {
	...
	public String getExistsNodeID(HttpServletRequest request, String json) throws Exception {
		...
		try {
			...
			String rootSql = "select NODEID " + " from FS_ORG_ROLE_TREE " + " where NODELEVEL='1' "
