...
public class WxyGcglMenuManager implements Cache {

	private static Logger logger = LoggerFactory.getLogger(WxyGcglMenuManager.class.getName());

	private Hashtable menutable;// 全部节点树 map<parent,List<MenuVo>>
	private Hashtable menuarr;// 全部菜单 map<name,MenuVo>

	private static WxyGcglMenuManager instance;
	private static final String GDZXT_APP_NAME = "wndjsgdwxygcgl";
	private static final String ROOT_MENU = "ROOT_MENU_85183803";

	private WxyGcglMenuManager() throws Exception {
		...
		try {
			...
			String querySql = "select name,title,parent,orderno,target,location,layersno,image,altimage,chief,app_name, levelno "
					...
					+ GDZXT_APP_NAME
					+ "' AND sfyx='1' order by parent,orderno ";
	...
	public MenuVo[] getMenus(String parent) throws Exception {
		try {
			if (Pub.empty(parent)){
				...
				return (MenuVo[]) ((ArrayList) menutable.get(parent)).toArray(new MenuVo[((ArrayList) menutable
						.get(parent)).size()]);
			}
		} catch (Exception e) {
			return null;
	...
	public synchronized static WxyGcglMenuManager getInstance() throws Exception {
		if (instance == null) {
			instance = new WxyGcglMenuManager();
