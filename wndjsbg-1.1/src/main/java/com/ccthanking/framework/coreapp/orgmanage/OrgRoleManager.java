...
public class OrgRoleManager implements Cache {
	...
	private static Logger log = LoggerFactory.getLogger(OrgRoleManager.class.getName());
	...
	private Role loadRole(String roleId) throws Exception {
		...
		try {
			conn = DBUtil.getConnection();
			// update by xhb on 2013-09-02
			String querySql = "select name,s_memo,level_name,deptid,role_id " + " from FS_org_role where role_id ='"
					+ roleId + "'";
			String[][] list = DBUtil.query(conn, querySql);
			if (list != null) {
				for (int i = 0; i < list.length; i++) {
...
//		}
		...
		try {
			conn = DBUtil.getConnection();
			// update by xhb on 2013-09-02
			String sql = "select role_name,person_account,role_id  from FS_org_role_psn_map where person_account='"
					+ account + "'";
	...
	public MenuVo[] getRoleMenus(String role_id) throws Exception {
		...
		try {
			...
			String sql = "select role_name,menu_name from FS_ORG_ROLE_MENU_MAP r, FS_EAP_MENU m"
					...
					+ "' and menu_name in (select NAME from fs_eap_menu where SFYX = '1'  AND  app_name='"
					+ Constants.APP_NAME + "' and levelno !=4 )" + " and m.levelno !=4 order by ORDERNO";
