...
@Controller
@RequestMapping("/fileUploadOldController")
public class FileUploadOldController {
    ...
    public requestJson deleteFile(HttpServletRequest request, requestJson js) throws Exception {
        ...
        FileUploadOldService.deleteByConditionVO(null, vo);
        j.setMsg(resultVO);
        return j;
    }
        ...
        try {
            ...
            if (vo!=null) {
				if (vo.getGlid4()!=null && "js".equals(vo.getGlid4())) {
					//建设系统附件
					syspara = (SysParaConfigureVO) ParaManager.getInstance().getSysParameter(Constants.getString(Constants.FILEUPLOADOLD_ROOT_JS, "FILEUPLOADOLD_ROOT_WNDJSJS"));
					fileRoot = syspara.getSysParaConfigureParavalue1();
				}
				if (vo.getGlid4()!=null && "jl".equals(vo.getGlid4())) {
					//监理库附件
					syspara = (SysParaConfigureVO) ParaManager.getInstance().getSysParameter(Constants.getString(Constants.FILEUPLOADOLD_ROOT_JL, "FILEUPLOADROOT_WNDJSJL"));
					fileRoot = syspara.getSysParaConfigureParavalue1();
				}
				if (vo.getGlid4()!=null && "gd".equals(vo.getGlid4())) {
					//工地附件
					syspara = (SysParaConfigureVO) ParaManager.getInstance().getSysParameter(Constants.getString(Constants.FILEUPLOADOLD_ROOT_GD, "FILEUPLOADOLD_ROOT_WNDJSGD"));
					fileRoot = syspara.getSysParaConfigureParavalue1();
				}
			}
            ...
            if (file.exists()) {
                ...
                File copyFile = new File(fileDir + File.separator + vo.getFilename());
				if (!copyFile.exists()) {
