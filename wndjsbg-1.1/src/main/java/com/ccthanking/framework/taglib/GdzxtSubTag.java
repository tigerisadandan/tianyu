...
public class GdzxtSubTag extends TagSupport {
		...
		if (menus_t != null && menus_t.length > 0) {
			for (int f = 0; f < menus_t.length; f++) {
				...
				for (int i = 0; i < menus.length; i++) {
					//------------------------------------------第一级菜单组成
					menuHtml += " <li id=\"" + menus[i].getName() + "\" class= > \r\n";//第一级菜单开始
					...
					if(menus_two != null && menus_two.length > 0){
						...
						menuHtml+=  "<a href=\"#\">";
						menuHtml += "<i class=\"icon-briefcase icon-white\"></i>&nbsp; <span class=\"menu-text\">" + menus[i].getTitle() + "</span> " ;
						menuHtml+= "</a>\r\n";
					}else{	
					...
					if (menus_two != null && menus_two.length > 0) {
						...
						for (int j = 0; j < menus_two.length; j++) {
							...
							*/
								String location=menus_two[j].getLocation();
								String newStr = "jsp"+location.substring(4, location.length())+".jsp";
								menuHtml += "<li class> " +
										" <a href=\"pagegdzxt/framework/gdzxt/frame_gdzxt_main#"+newStr+"\" " +
										" data-url=\""+newStr+"\" " +
										" onclick=\"menutree_click('" 
											+ menus_two[j].getName() + "','"
											+ newStr + "','" 
...
//		System.out.println(menuTreeHtml);
	...
	protected MenuVo[] getAllowedMenus(User user, String parent) throws Exception {
		...
		if (User.SUPER_USER.equals(user.getPersonKind())) {
			return GdzxtMenuManager.getInstance().getMenus(parent);
		} else {
			return GdzxtMenuManager.getInstance().getGDZXTMenus(user, parent);
