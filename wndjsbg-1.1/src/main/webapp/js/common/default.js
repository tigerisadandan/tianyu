...
defaultJson.doQueryJsonList = function(actionName, data1,tablistID,callbackFunction,noAlertnoResult){
				...
				if(returnMsg=="0")
				{
					if(noalert==true){
						...
						if(g_bAlertWhenNoResult){
						  //--begin----查询数据为空时，IE10浏览器兼容添加空行
							var rows = $("tbody tr" ,$("#"+tablistID.id));
							if(rows.size()==0){
								$("tbody" ,$("#"+tablistID.id)).append("<tr style=\"height: 1px;border:none;\"></tr>");
							}
						 //---end----	
						    doInitPages(tablistID);
	 ...
	 * param:  noAlertnoResult  没有查询结果是否提示信息
	 */
	defaultJson.doQueryJsonListReturn = function(actionName, data1,tablistID,callbackFunction,noAlertnoResult){
		 var returnobj=null;
		//function doQueryJson(actionName, data1) {
	       var success  = true;
	       var isAsync = false;
	...
	doQueryPage = function(actionName, queryconfig,tablistID,resultJson){
    	...
    	var tableid = tablistID.id;
   	    ...
   	    var pageHTML  ="<form class=\"form-inline\" currentpagenum='"+currentpagenum+"'>";
	    	...
	    	if($(tablistID).attr("print")=="true"){
				var pageHTML ="<span class=\"pull-left\">";
			 	pageHTML +="<button id=\"btnPrint\" class=\"btn btn-link\" type=\"button\" onclick=\"doExecl('"+tablistID.id+"')\"><i class=\"icon-print\"></i>导出</button></span>";
			 	$("#page_"+tablistID.id).find("form").eq(0).append(pageHTML);
			}
	...
	doInitPages  = function (tablistID){
	 ...
	 var pageHTML  ="<form class=\"form-inline\">";
	 	 pageHTML +="<span class=\"pull-right\">";
	 	 pageHTML +="<button id=\"btnExpExcel\" class=\"btn\" type=\"button\" style=\"font-family:SimYou,Microsoft YaHei;font-weight:bold;\">导出</button></span>";
		 pageHTML +="<button type=\"button\" disabled class=\"btn btn-mini btn-link\" title=\"首页\" ><i class=\"icon-fast-backward\"></i></button>";
		 pageHTML +="<button type=\"button\" disabled class=\"btn btn-mini btn-link\" title=\"上一页\"><i class=\"icon-step-backward\"></i></button>";
		 pageHTML +="[0/0]";
		 pageHTML +="<button type=\"button\" disabled class=\"btn btn-mini btn-link\" title=\"下一页\"><i class=\"icon-step-forward\"></i></button>";
...
//		});
   ...
   //function doExecl(tabId,actionName){
	//   $(window).manhuaDialog({"title":"导出","type":"text","content":"/wndjsbg/jsp/framework/print/TabListEXP.jsp?tabId="+tabId+"&actionName="+actionName,"modal":"3"});
  // } 
...
function xmscUrl(id){
	 var  xmsc = {"title":"项目手册","type":"text","content":g_sAppName+"/jsp/business/jhb/xdxmk/rowView.jsp?id="+id,"modal":"1"};
	 return xmsc;
}
...
function generateFyjjhNdMc(ndObj,mcObj){
	ndObj.attr("src","T#GC_JH_SJ: distinct ND:ND:SFYX='1'");
	ndObj.attr("kind","dic");
	ndObj.html('');
...
function queryProjectWithBD(paras){
	$(window).manhuaDialog({"title":"项目列表","type":"text","content":g_sAppName + "/jsp/business/gcb/kfglgl/xmcx.jsp?"+paras,"modal":"4"});
...
function sendMsg(title) {
	...
	var url = g_sAppName+"/jsp/business/xtbg/msg/addNbdx.jsp";
