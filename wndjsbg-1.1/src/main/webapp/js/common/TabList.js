...
//			alert("tempjsonaaa:"+tempjson);
						...
						if(Thtype == undefined){//不可编辑
							...
							switch(Thtype.toLowerCase())
	        				{
	        					...
	        					case 'dic':
	        	        			...
	        	        			if(htsrc != undefined)
	    	        				{
	        	        				showHTML +="<td><select  class=\"span12\" type= \"select-one\" name = \""+val+"\" fieldname= \""+val+"\" kind=\"dic\" src=\""+htsrc+"\" "+noNullSelect+"></select></td>";
	    	        				}
	        	        			break;
	        					case 'date':
 ...
 //   	var tabletype = tableObject.attr("type");
    	var tbHead = tableObject.children('thead');//获取table对象下的thead 
    	var tbHeadTh_tmp = tbHead.find('tr th');//获取thead下的tr下的th 
    	var tbHeadTh = new Array();
    	var th_leng = 0;
    	$(tbHeadTh_tmp).each(function(i){
		   if(!$(this).is(':hidden')){
			   tbHeadTh[th_leng]=$(this);
			   th_leng++;
		   }
		});
    	
    	tbHeadTh = jQuery(tbHeadTh);
...
//    	alert(oS.find('option:selected').text());
    	eValue += ',"'+oS.find('option:selected').text()+'"';
    	if (eValue != "")
    	{
