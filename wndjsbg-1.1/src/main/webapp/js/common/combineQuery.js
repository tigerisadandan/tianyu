...
combineQuery.getQueryCombineData = function (queryform,orderform,tableid) {
	...
	if(tableid)
	{
		//begin-----根据浏览器窗口大小，调节查询数据的条数，屏蔽下面的代码
		...
		//}	
		//修改成以下方式
		var nopage= $("#"+tableid.id).attr("noPage");
		//为了适应不需要分页的情况，做如下处理，nopage为true时 取设置的参数值，空的话就采用自适应的方式取值
		...
		if(nopage){
			...
			//为了区别某些特殊的页面，需要固定其页码数，故添加改参数，为true时，取自定义的页码
			var noheight= $("#"+tableid.id).attr("noheight");
			if(noheight){
				pageNum = $("#"+tableid.id).attr("pageNum");
				if(pageNum==null||pageNum==""){
					pageNum="10";
				...
				$("#"+tableid.id).attr("pageNum",pageNum);
			}	
		}
		//---end-----
	}
