...
<html>
	<head>
		...
		<script type="text/javascript" charset="utf-8">
			var loadBelong = ''; //附件从属
			var btnObj = null;
			var fileUid = null; //储存附件UID的input对象
			//----------------------------------------
			//-重新定义iframe大小
			//----------------------------------------
			...
			//-点击表格中“添加文件”按钮的事件  
			//obj  上传按钮对象
			//belong    'parent' 从属于主表的附件  
			//			'child'  从属于子表的附件
			//----------------------------------------
			function doSelectFile(obj,belong){
				loadBelong = belong;
				btnObj = obj;
				var inputArr = $(".myKeyValueDiv input");
					
				if(belong=='parent'){
					fileUid = $("#"+$(obj).attr("business_type")+"_FILEUPLOAD");
				}else if(belong=='child'){
					fileUid = $(obj).closest('tr').find("#"+$(obj).attr("business_type")+"_FILEUPLOAD");
				}
				for(var xx=0;xx<inputArr.length;xx++){
					if(inputArr[xx].getAttribute("cond")=="true"){
						if(inputArr[xx].getAttribute("condName")=="ywid"){
							if(belong=='parent'){
								inputArr[xx].value=$("#"+$(obj).attr("ywid")).val();
							}else if(belong=='child'){
								inputArr[xx].value=$(obj).closest('tr').find("#"+$(obj).attr("ywid")).val();
							}
						}
					}
					if(inputArr[xx].getAttribute("cond")=="true"){
						if(inputArr[xx].getAttribute("condName")=="target_type"){
							inputArr[xx].value=$(obj).attr("target_type");
						}
					}
					if(inputArr[xx].getAttribute("cond")=="true"){
						if(inputArr[xx].getAttribute("condName")=="file_type"){
							inputArr[xx].value=$(obj).attr("file_type");
						}
					}
					if(inputArr[xx].getAttribute("cond")=="true"){
						if(inputArr[xx].getAttribute("condName")=="business_type"){
							inputArr[xx].value=$(obj).attr("business_type");
						}
					}
				}
			...
			function optionsHandler(obj){
				...
				obj.acceptFileTypes =/(\.|\/)(gif|jpe?g|png|bmp|pdf)$/i;//只许上传这些文件
				return obj;
			}
			...
			function insertFileTab(result){
				...
				var tab_width = 0;
					var showFile = $(btnObj).parent().find(".showFileTab");
					tab_width = $(showFile).parent().parent().width();
					$(showFile).parent().addClass("nomargin");
					var showType = $(showFile).attr("showType");
					if(showType!=undefined){
						insertImageTab($(showFile),files,tab_width);
					}else{
						
						insertDetailTab($(showFile),files,tab_width);
					}
			}
			//----------------------------------------
			//-预览图片
			...
			function setFileData2(ywid,beLong){
				var inputArr = $(".myKeyValueDiv input");
				//if($("#ywid").val()==undefined || $("#ywid").val()==""){
				$("#ywid").val(ywid);
				//}
			...
			var optype = "";
			function queryFileData2(ywid,belong,business_type){
				var at_fileuid = '';
				optype = "query";
				$("input[id='"+ywid+"']").each(function(){
					if($(this).val()!=null&&$(this).val()!=""){
						at_fileuid += ($(this).val()+",");
					}
				})
				var obj = new Object();
				obj.TARGET_UID = at_fileuid;
 				...
 				$.ajax({
					...
					success : function(result) {
						...
						$.each(fileList,function(index,file){
							$("span[id='addFileSpan']").each(function(i,demo){
								var _file = null;
								//区分父子表附件,分别取关联值
								if(belong=='parent'){
									_file = $("#"+ywid).val();
								}else if(belong=='child'){
									_file = $(demo).parent().parent().parent().find('#'+ywid).val();
								}
								
								if(typeof(_file)!='undefined'&&_file==file.TARGET_UID&&$(demo).attr("file_type")==file.FILE_TYPE&&$(demo).attr("business_type")==file.BUSINESS_SUB_TYPE){
									
									btnObj = demo;				//设置为当前选中按钮,模拟执行文件选中后的 显示文件列表操作
									//clearFileTab("query");
									insertFileTab("["+JSON.stringify(file)+"]");
								}
							})
						})
			...
			function insertDetailTab(obj,files,tab_width){
				...
				for(var i=0;i<files.length;i++){
					...
					$(fileUid).val($(fileUid).val()+(file.TARGET_UID+","))
					...
					if(true){
						...
						*/
						...
						var className = "";
						if(optype=="query"){
							className = "my-showPreview-btn nameSpan";
						}else{
							className = "";
						}
						//名字长度过长，折行处理
						if(file.name.length>g_nameMaxlength){
							...
							//showHtml +="<span class='my-showPreview-btn nameSpan' fileid='"+file.AT_FILEUPLOAD_UID+"'><abbr title='"+file.name+"'>"+file.name.substring(0,g_nameMaxlength)+"...</abbr></span>"
							showHtml +="<span class='"+className+"' fileid='"+file.AT_FILEUPLOAD_UID+"'><abbr title='"+file.name+"'>"+file.name.substring(0,g_nameMaxlength)+"...</abbr></span>"
						}else{
							//showHtml +="<span class='my-showPreview-btn nameSpan' fileid='"+file.AT_FILEUPLOAD_UID+"' target='_blank'>"+file.name+"</span>"
							showHtml +="<span class='"+className+"' fileid='"+file.AT_FILEUPLOAD_UID+"' target='_blank'>"+file.name+"</span>"
						}
						showHtml +="</td>";

						showHtml +="<td class='otherColumns' style='border:0;padding:0px;'>"+file.CREATED_DATE+"</td>";
						...
						if(obj.attr("onlyView")=="true"){
							...
							if(obj.attr("deleteUser")!=undefined && obj.attr("deleteUser")==file.lrrCode){
								...
								showHtml +="<button class='btn btn-link my-del-btn' title='删除' data-type='"+file.delete_type+"' data-url='"+file.delete_url+"' >";
						...
						showHtml +="<a href='javascript:void(0)' title='下载' onclick='loadFiles(\""+file.url+"\")' class='btn btn-link' fileid='"+file.fileid+"' download='"+file.name+"'>";
				...
				for(var i=0;i<files.length;i++){
					...
					if(true){
						if(file.fileType.indexOf("image")!=-1){
							...
							showHtml +="<i class='icon-trash'></i>";
							showHtml +="<span  style='font-size:12px;'>删除</span>";
							showHtml +="</button>";
							showHtml +="</td>"
							showHtml +="</tr>";
							if(_attr=="multi"){
	...
	<body>
		<div class="row-fluid">
			<!-- The file upload form used as target for the file upload widget -->
				...
				<div id="fileupload" style="position:absolute;top:200px;">
				...
				<table role="presentation" class="table table-striped" id="db">
		...
		<script src="${pageContext.request.contextPath }/file_upload/js/cors/jquery.xdr-transport.js"></script>
		<![endif]-->
		
		<div style="display:none;" class="myKeyValueDiv">
			<input type="hidden" cond="true" condName="ywid">
			<input type="hidden" cond="true" condName="target_type">
			<input id="file_type" type="hidden" cond="true" condName="file_type">
			<input id="business_type" type="hidden" cond="true" condName="business_type">
			...
			<input type="hidden" cond="true" condName="enabled">
