...
function insertDetailTab(obj,files,tab_width){
	...
	for(var i=0;i<files.length;i++){
		...
		if(file_type==undefined || file_type==file.FILE_TYPE){
			showHtml +="<tr class='template-download'   rowid='"+file.FS_FILE_UID+"'>";
			showHtml +="<td class='name' style='border:0;padding:0px;'>";
			//名字长度过长，折行处理
			if(file.FILE_NAME.length>30){
				showHtml +="<span class='my-showPreview-btn nameSpan' style='cursor:hand' fileid='"+file.FS_FILE_UID+"'><abbr title='"+file.FILE_NAME+"'>"+file.FILE_NAME.substring(0,g_nameMaxlength)+"...</abbr></span>"
			}else{
				showHtml +="<span class='my-showPreview-btn nameSpan' style='cursor:hand' fileid='"+file.FS_FILE_UID+"' target='_blank'>"+file.FILE_NAME+"</span>"
			}
			showHtml +="</td>";
			if(obj.attr("showSize")!="false"){
				showHtml +="<td class='size ' style='border:0;padding:0px;'>"+file.FILE_SIZE+"KB</td>";
			}
...
// 								showHtml +="<td class='myPreview' style='border:0;text-align:center;padding:0px;'>";
			...
			showHtml +="<a href='"+file.url+"' class='btn btn-link' fileid='"+file.FS_FILE_UID+"' download='"+file.FILE_NAME+"'>";
	...
	for(var i=0;i<files.length;i++){
		...
		if(file_type==undefined || file_type==file.FILE_TYPE){
			if(file.fileType.indexOf("image")!=-1){
				...
				if(_preview=="true"){
					showHtml +="<a href='javascript:void(0)' class='my-showPreview-btn nameSpan' fileid='"+file.FS_FILE_UID+"'><img src='"+file.url+"' style='"+_width+_height+"'></a>";
				}else{
