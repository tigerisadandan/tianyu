...
<html>
	<head>
		...
		<script type="text/javascript" charset="utf-8">
			...
			function insertFileTab(result){
				...
				$(".showFileTab").each(function(i){
					...
					if(showType!=undefined){
						...
						var glid1 = $(this).attr("glid1");
						
						if(glid1!=undefined){
							insertDetailTab_Glid1($(this),files,tab_width);
						}else{
			...
			function insertDetailTab_Glid1(obj,files,tab_width){
				var fjlb = obj.attr("fjlb");
				var glid1 = obj.attr("glid1");
				for(var i=0;i<files.length;i++){
					var showHtml = "";
					var file = files[i];
					//对关键字段赋值
					...
					//alert("glid1---"+glid1+"------------------file.glid1-----"+file.glid1);
					var number = 0;
					//如果table没有附件类别，那么插入所有数据，否则只插入附件类别相匹配的数据
					if(fjlb==file.fjlb && glid1==file.glid1){
						if(tab_width<600){
							showHtml +="<table class='fu_blockTable_full' style='display:inline-block;' cellspacing='0' cellpadding='0'>";
						}else{
							showHtml +="<table class='fu_blockTable_half' cellspacing='0' cellpadding='0'>";
