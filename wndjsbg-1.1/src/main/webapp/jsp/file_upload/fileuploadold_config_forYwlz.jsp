...
<html>
	<head>
		...
		<script type="text/javascript" charset="utf-8">
			...
			function clearFileTab2(n,obj){
					if($(obj).attr("notClear")=="true"){
						//doNothing
					}else{
						$(obj).empty();
					}
			...
			function insertFileTab(result){
				...
				var tab_width = 0;
					...
					tab_width = $("#"+currYwcl).parent().parent().width();
					$("#"+currYwcl).parent().addClass("nomargin");
					var showType = $("#"+currYwcl).attr("showType");
					if(showType!=undefined){
						insertImageTab($("#"+currYwcl),files,tab_width);
					}else{
						insertDetailTab($("#"+currYwcl),files,tab_width);
					}
			}
			function insertFileTab2(result,curr){
				var files = eval('(' + result + ')');
				if(g_nameMaxlength==undefined){
					g_nameMaxlength=12;
				}
				var tab_width = 0;
					...
					tab_width = $(curr).parent().parent().width();
					$(curr).parent().addClass("nomargin");
					var showType = $(curr).attr("showType");
					if(showType!=undefined){
						insertImageTab($(curr),files,tab_width);
					}else{
						insertDetailTab($(curr),files,tab_width);
					}
			}
			//----------------------------------------
			//-预览图片
			...
			function insertDetailTab(obj,files,tab_width){
				...
				for(var i=0;i<files.length;i++){
					...
					//如果table没有附件类别，那么插入所有数据，否则只插入附件类别相匹配的数据
					if(true){
						showHtml +="&nbsp;<span class='template-download' rowid='"+file.fileid+"'>";

<%--						showHtml +="<a href='"+file.url+"' class='btn btn-link' fileid='"+file.fileid+"' download='"+file.name+"'>";--%>
<%--						showHtml +="<i class='icon-download-alt'></i>";--%>
<%--						showHtml +="<span>"+file.name+"</span>";--%>
<%--						showHtml +="</a>&nbsp;";--%>
						
						if(file.name.length>g_nameMaxlength){
							showHtml +="<span class='my-showPreview-btn nameSpan' fileid='"+file.fileid+"'><abbr title='"+file.name+"'><a href=\"javascript:void(0)\">"+file.name.substring(0,g_nameMaxlength)+"...</a></abbr></span>"
						}else{
							showHtml +="<span class='my-showPreview-btn nameSpan' fileid='"+file.fileid+"' target='_blank'><a href=\"javascript:void(0)\">"+file.name+"</a></span>&nbsp;"
						}
						
						showHtml +="<span class='delete ' style='border:0;padding:0px;'>";
						if(!false){
	...
	// 								showHtml +="<td class='myPreview' style='border:0;text-align:center;padding:0px;'>";
			        ...
			        showHtml +="<i class='icon-download-alt'></i>"; //下载
			        showHtml +="<span>下载</span>";
			        showHtml +="</a>&nbsp;";
	...
	<body>
		...
		<div style="display:none;" class="myKeyValueDiv">
			...
			<input type="hidden" cond="true" condName="glid4" value="js">
