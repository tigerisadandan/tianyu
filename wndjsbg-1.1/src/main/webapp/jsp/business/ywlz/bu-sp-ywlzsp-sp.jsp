...
var ifbz=false; //32业务 是否 最后一步骤 

function xzjdxz(){
	$("#ajrytb").show();
	$.ajax({
		url : "${pageContext.request.contextPath}/ywlz/buSpYwlzBzController?queryJlRy",
	//	data : data1,
		cache : false,
		async : false,
		dataType : "json",
		...
		success : function(response) {
			...
			jdz=res;
			var showHtml="<select  onchange=\"selJdz(this)\" id='jdxzSel'><option value=\"0\">--请选择监督组--</option>";
			for(var i=0;i<res.response.data.length;i++){
				if(res.response.data[i].ORG_TYPE=="D"){
					showHtml+="<option value=\""+res.response.data[i].ORGANIZE_UID+"\">"+res.response.data[i].ORG_NAME+"</option>";
				}
			}
			showHtml+="</select>";
			$("#selJdz").html(showHtml);
			}
		});
}
var jdz=null;
...
function selJdz(obj){
	var showHtml="";
	for(var i=0;i<jdz.response.data.length;i++){
		if(jdz.response.data[i].P_ORGANIZE_UID==$(obj).val()){
			jdzid=$(obj).val();
			showHtml+="<tr><td><input type=\"radio\" name='jlry' value=\""+jdz.response.data[i].USER_UID+"\" CODE=\""+jdz.response.data[i].CODE+"\" /></td><td>"+jdz.response.data[i].ORG_NAME+"</td><td>"+jdz.response.data[i].PHONE+"</td></tr>";
		}
	}
	$("#jlry").html(showHtml);
}
...
function addJdxz(){
	var jlr="";
	$("#jlry").find("input").each(function (i,item){
		if($("input[name='jlry']:checked").val()==$(item).val()){
			jlr+=$(item).val()+":Y:"+$(item).attr("CODE");
		}else{
		    jlr+=$(item).val()+":N:"+$(item).attr("CODE");
		}
		if(i<$("#jlry").find("input").length-1){
			jlr+=",";
		}
		
	})
	
	defaultJson.doInsertJson("${pageContext.request.contextPath}/ywlz/buSpYwlzBzController?insertJlRy&strJl="+jlr+"&strgcid="+wj_PROJECTS_UID+"&jdxzid="+$("#jdxzSel").val(), null);
}
...
function init(){
		...
		//调用ajax插入
		var res = defaultJson.doQueryJsonListReturn("${pageContext.request.contextPath}/ywlz/buSpYwlzBzController?query", dataXX, spyjList);
        if(res.msg!="0"){
        	ifbz=true;
        }
	...
	if(sfsx=='1'){
		...
		$(function() {		      					
				$("#btnAddYj").click(function() {
					...
					if(spywid=="32"&&ifbz){
					   if($("input[name='jlry']:checked").val()==undefined){
						   alert("请选择监督组总监！");
						   return;
					   }else{
						   addJdxz();
							 ...
							 <div class="overFlowX">
							  <form id="spyjForm">
									<table class="B-table" width="100%"  id="spyjtableid">
										...
										<tr id="ajrytb" style="display: none;">
								            <td colspan="10">
								            <table class="B-table" width="100%"  id="spyjtableid" >
									  			<input type="hidden"  id="LZBZ_UID_ID" fieldname="LZBZ_UID" name="LZBZ_UID"/>
												<input type="hidden"  id="YWLZ_UID_ID" fieldname="YWLZ_UID" name="YWLZ_UID"/>
												...
												<input type="hidden"  id="SFSX_ID"  name="SFSX_ID"/>
										<tr >
											<th width="1%" class="right-border bottom-border text-right">选择监督小组</th>
											<td width="20%" class="right-border bottom-border" id="selJdz" ></td>
