...
$(function() {
	...
	$("#btnUpdate").click(function() {
		...
		$(window).manhuaDialog({"title":"建设项目>审核","type":"text","content":"${pageContext.request.contextPath }/jsp/business/project/js-project-add.jsp?type=update","modal":"2"});
	});
	//按钮绑定事件（导出EXCEL）
...
function rowView(obj,JSRW_Y){
	...
	$("#resultXML").val(data);
	if(JSRW_Y==1){  //建设项目
	$(window).manhuaDialog({"title":"建设项目>详细信息","type":"text","content":"${pageContext.request.contextPath }/jsp/business/project/js-project-add.jsp?type=detail","modal":"2"});
	}else if(JSRW_Y==0){//环保项目
		$(window).manhuaDialog({"title":"环保项目>详细信息","type":"text","content":"${pageContext.request.contextPath }/jsp/business/project/js-project-add-hb.jsp?type=detail","modal":"2"});
	}

}


function rowSh(obj,JSRW_Y){
	while(obj.tagName.toLowerCase() != "tr"){
	    obj = obj.parentNode;
	    if(obj.tagName.toLowerCase() == "table")return null;
	   }
	...
	if(JSRW_Y==1){  //建设项目
		$(window).manhuaDialog({"title":"建设项目>审核","type":"text","content":"${pageContext.request.contextPath }/jsp/business/project/js-project-add.jsp?type=update","modal":"2"});
			...
			$(window).manhuaDialog({"title":"环保项目>审核","type":"text","content":"${pageContext.request.contextPath }/jsp/business/project/js-project-add-hb.jsp?type=update","modal":"2"});
		}
...
function caozuoFun(obj){
	...
	var showHtml="";
	if(status=='30'){
		showHtml +="<span class='btn btn-link' id='addSpan' onclick='rowSh(this,"+obj.JSRW_Y+");' title='审核' >";
		showHtml +="<i class='icon-file'></i> ";
		showHtml +="</span>";
	}else{
		showHtml +="<span class='btn btn-link' id='addSpan' onclick='rowView(this,"+obj.JSRW_Y+");' title='查看' >";
		showHtml +="<i class='icon-file'></i> ";
		showHtml +="</span>";
	}
	return showHtml;
...
function xmbhFun(obj){
	...
	if($("input[name='status']:checked").val()==10){
		return showHtml = "<a href='javascript:void(0);' title='项目编号:"+obj.PROJECT_CODE+"'>"+obj.GONGCHENG_NAME+"</a>";
	}else{
		return "";
		}
}
...
function getCount(){	
	$.ajax({
		url : controllername+"?getProjectCount",
	//	data : data1,
		cache : false,
		async : false,
		dataType : "json",
	            ...
	            <table width="100%" class="table-hover table-activeTd B-table" id="DT1" type="single"  pageNum="18">
	                <thead>
	                	<tr> 
	                		...
	                		<th fieldname="PROJECT_UID" colindex=2 tdalign="center" CustomFunction="caozuoFun" >&nbsp;&nbsp;</th>
							<th fieldname="COMPANYNAME" colindex=2 tdalign="center" >&nbsp;公司名称&nbsp;</th>
							<th fieldname="GONGCHENG_NAME" colindex=5 tdalign="center" maxlength="30" CustomFunction="xmbhFun">&nbsp;项目名称&nbsp;</th>
							<th fieldname="GONGCHENG_NAME" colindex=5 tdalign="center" maxlength="30" CustomFunction="xmType">&nbsp;项目类型&nbsp;</th>
							...
							<th fieldname="TOUZI_XINGZHI" colindex=3 tdalign="center" >&nbsp;投资性质&nbsp;</th>	
							...
							<th id="shrid" fieldname="SHENHENAME" colindex=9 tdalign="center" maxlength="30" >&nbsp;审批人&nbsp;</th>
							<th id="shsjid" fieldname="SHENHE_DATE" colindex=10 tdalign="center" >&nbsp;审批日期&nbsp;</th>
							<th fieldname="JSRW_Y" colindex=11 hidden=""></th>
	                	</tr>
	                </thead>
	              	<tbody></tbody>
	           </table>
