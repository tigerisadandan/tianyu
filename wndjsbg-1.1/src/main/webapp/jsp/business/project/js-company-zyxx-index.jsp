		...
		// alert(status);
		 if(status=='30'){
			// alert(status);
				...
				$("#shsjid").hide();
				$("#tijiao").show();	
				$("#txtFilter_sj").attr("fieldname","JS_COMPANY_UID");
			}else{
				$("#shrid").show();			
				$("#shsjid").show();
				$("#tijiao").hide();	
...
function rowView(obj){
	...
	$("#resultXML").val(data);
	$(window).manhuaDialog({"title":"建设单位>详细信息","type":"text","content":"${pageContext.request.contextPath }/jsp/business/project/js-company-zyxx-add.jsp?type=detail","modal":"2"});
}
...
function rowSh(obj){
	...
	$("#resultXML").val(data);
	$(window).manhuaDialog({"title":"建设单位>详细信息","type":"text","content":"${pageContext.request.contextPath }/jsp/business/project/js-company-zyxx-add.jsp?type=update","modal":"2"});
}


function caozuoFun(obj){
	var status=obj.ZYXX_STATUS;
	var showHtml="";
	if(status=='30'){
...
function getCount(){
	$.ajax({
		url : controllername+"?getCompanyZyxxCount",
	//	data : data1,
		cache : false,
		async : false,
		dataType : "json",
				...
				<table class="B-table" width="100%">
					...
					<TR style="display: none;">
						...
						<TD class="right-border bottom-border">
							...
							<INPUT type="text" class="span12" id="QSTATUS" fieldname="ZYXX_STATUS" value="30"  operation="="/>
						</TD>
					</TR>
					<!--可以再此处加入hidden域作为过滤条件 -->
					<tr>
	            ...
	            <table width="100%" class="table-hover table-activeTd B-table" id="DT1" type="single"  pageNum="18">
	                <thead>
	                	<tr>
							...
							<th fieldname="WAIDI_Y" colindex=5 tdalign="center" maxlength="30" CustomFunction="sfwd">&nbsp;是否为外地企业&nbsp;</th>
