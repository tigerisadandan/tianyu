...
$(function() {
	...
	window.open("${pageContext.request.contextPath}/jsp/business/bzwj/bu_sp_bzwj-add.jsp?type=insert&SPBZ_UID="+uid);
...
function setFormValues(){
	...
	$.ajax({
		...
		success : function(response) {
			...
			$("#SPYW_UID").val(resultobj.SPYWMC);
			$("#SPYW_UID").attr("code",resultobj.SPYW_UID);
		}
	});
}
//查询出所有人员
	...
	$.ajax({
		...
		success : function(response) {
			...
			$(arr).each(function(i){
				...
				showHtml += "<input type='checkbox' name='cyz' onChange='addUsersC(this)' id='cyz' value='"+this.USERS_ID+"' text='"+this.USER_NAME+"' "+checked+"/>";
...
function submitUsers(){
	...
	$("input:checkbox[name='cyz']:checked").each(function(){
		showHtml += "<span id='_"+$(this).val()+"' style='padding-right:10px;'>";
...
function addUsersC(demo){
	//alert($(demo)[0].checked);
	if($(demo)[0].checked){
		var showHtml = "";
		showHtml += "<span id='_"+$(demo).val()+"' style='padding-right:10px;'>";
		showHtml += $(demo).attr("text");
		showHtml += "<input type='hidden' id='user_"+$(demo).val()+"' value='"+$(demo).val()+"' fieldname='USERS_UID'>";
		//showHtml += "<a href='javascript:void(0)' onclick='removeCurr(this)'>";
		//showHtml += "<i class='icon-remove-sign'></i>";
		//showHtml += "</a>";
		showHtml += "</span>";
		...
		//alert(234);
		$("#_"+$(demo).val()).closest("span").remove();
	}
	
}
function clearPersonName(){
	$("#uname").val("");
	addUsers();
}
//查询参与者，并显示
function queryUsers(){
	$.ajax({
		url : controllernameCyz+"?queryBzCyz&uid="+$("#SPBZ_UID").val(),
		data : {},
		cache : false,
		async :	false,
		dataType : "json",  
		...
		success : function(response) {
			if(response.msg!=""){
				...
				var showHtml = "";
				$(arr).each(function(){
					showHtml += "<span  id='_"+this.USERS_ID+"'  style='padding-right:10px;'>";
					showHtml += this.USER_NAME;
					showHtml += "<input type='hidden' id='user_"+this.USERS_ID+"' value='"+this.USERS_ID+"' fieldname='USERS_UID'>";
					//showHtml += "<a href='javascript:void(0)' onclick='removeCurr(this)'>";
					//showHtml += "<i class='icon-remove-sign'></i>";
					//showHtml += "</a>";
					showHtml += "</span>";
...
<div class="row-fluid">
      ...
      <!--可以再此处加入hidden域作为过滤条件 -->
      	<TR  style="display:none;">
			...
			<TD class="right-border bottom-border">
				<input type="text" id="QSPBZ_UID" name="SPBZ_UID" fieldname="t.SPBZ_UID" operation="=" />
			</TD>
        </TR>
      </table>
      </form>
	...
	<div class="B-small-from-table-autoConcise">
     ...
     <form method="post" id="buSpYwxxForm">
      ...
      <input type="hidden"  fieldname="USERS_UID" /></TD>
        <tr>
		...
		<tr>
			<th width="8%" class="right-border bottom-border text-right">处理天数</th>
       		<td class="bottom-border right-border"width="15%">
         		<input class="span12" style="width:85%" id="CLTS" type="text" placeholder="必填" check-type="required" fieldname="CLTS" name = "CLTS"/>
         	</td>
		...
		<tr>
			<th width="8%" class="right-border bottom-border text-right">所属业务</th>
         	...
         	<th width="8%" class="right-border bottom-border text-right">是否有效</th>
       		<td class="bottom-border right-border"width="15%">
         		<input class="span12" style="width:50%" id="ENABLED" type="radio" check-type="required" kind="dic" src="SF" fieldname="ENABLED" name = "SF" />
         	</td>
		</tr>
		 <tr>
				...
				<div id="userBlock" style="margin-top: 5px">
					...
					<tr>
						...
						<td class="text-left bottom-border text-right">
	                        ...
	                        <button id="btnClear"  onClick="clearPersonName()" class="btn btn-link"  type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;">清空</button>
      ...
      <form method="post" id="bzwjQuery"  >
				<input class="span12" type="hidden" id="qqSPBZ_UID" name = "SPBZ_UID" fieldname = "SPBZ_UID" operation="=" >
      </form>
