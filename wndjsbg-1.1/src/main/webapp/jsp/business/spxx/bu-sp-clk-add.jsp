...
function init(){
	...
	if (type == "insert") {
		...
		$("input[name='SFYFJ'][value='0']")[0].checked=true;
	} else if (type == "detail") {
		//生成json串
...
//alert("---data---"+data);		
		...
		$.ajax({
			...
			setFileData(resultobj.CLK_UID, "", "", "", "1","");
			return true;
			}
		});
...
function sfyfj(sfyfj){
	 var jgvalues=$("input[name='SFYFJ']:checked").val();
	if(jgvalues=='1'){
		$("#addFileSpan").show();
	}else{
		$("#addFileSpan").hide();
...
<%--				<button id="btnClear_Bins" class="btn" type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;">清空</button>--%>
     ...
     <form method="post" id="buRequirementsForm" >
      <table class="B-table" width="100%" >
      		...
      		<input type="hidden" id="CLK_UID" fieldname="CLK_UID" name="CLK_UID"/>
	    <tr>
        ...
        <tr>
			<th width="8%" class="right-border bottom-border text-right">排序号</th>
       	 	<td class="bottom-border right-border" width="23%">
         		<input type="text" name="SERIAL_NO" id="SERIAL_NO" fieldname="SERIAL_NO" check-type="required" maxlength="4" style="width: 90%;" />
       	 	</td>
         ...
         <tr>
         	<th width="8%" class="right-border bottom-border text-right">是否有附件</th>
       		<td class="bottom-border right-border"width="15%">
       			<input class="span12" id="SFYFJ_ID" onclick="javascript:sfyfj(this)" type="radio" check-type="required" fieldname="SFYFJ" name = "SFYFJ" kind="dic" src="SF" />
         	</td>
        ...
        <tr id="sfyfjtrid">
			...
			<td colspan="3" class="bottom-border right-border">
				<div>
					<span class="btn btn-fileUpload" id="addFileSpan" onclick="doSelectFile(this);" fjlb="1607"  >
