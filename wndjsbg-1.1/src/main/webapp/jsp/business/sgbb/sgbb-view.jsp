...
var controllernameGctype= "${pageContext.request.contextPath }/sgbb/gcTypeController/";
var controllernamePersonZs = "${pageContext.request.contextPath }/person/sgZhengshuController/";
var controllernameRy = "${pageContext.request.contextPath }/sgbb/sgbbRyController/";
var stemp = 0;
...
<%--		$(window).manhuaDialog.close();--%>
    ...
    $("#btnPrint").click(function(){
       	window.open("${pageContext.request.contextPath }/sgbb/sgBbController/query4Print?bbid="+$("#QID").val()+"&type=pdf","打印PDF");
<%--    	$(window).manhuaDialog({"title":"导出","type":"text","content":"${pageContext.request.contextPath }/jsp/business/sgbb/print.jsp?bbid="+$("#ID").val(),"modal":"3"});--%>
    })

    $("#btnPass").click(function() {
        var flag = confirm("确定中标吗!");
        if(!flag){return;}
        var bbid = $("#SGBB_UID").val();
...
function setFormValues(){
	...
	$.ajax({
		...
		success : function(response) {
			...
			if(resultobj.CB_XINGZHI=="0"){
				$("#CB_XINGZHI").val("施工总承包");
			}else if(resultobj.CB_XINGZHI=="1"){
				$("#CB_XINGZHI").val("专业承包");
			}
...
function dozt(obj){
	if($("#STA").val()==20||$("#STA").val()==50){
		if(obj.STATUS==1){//解锁
			return "<a href='javascript:void(0)' onclick=lockOp("+obj.SGBB_RY_UID+",'doBbry')><img src='${pageContext.request.contextPath }/images/sgxt/lock.png' title='锁定中,点击解锁'/></a>";
		}
		else{//锁定
			return "<a href='javascript:void(0)' onclick=personLock("+obj.SGBB_RY_UID+")><img src='${pageContext.request.contextPath }/images/sgxt/lock-unlock.png' title='锁定人员'/></a>";
			}
	}
	else if($("#STA").val()==2){
		if(obj.STATUS==1){//解锁
	...
	<div class="row-fluid">
		...
		<div class="B-small-from-table-autoConcise">
			<form method="post" id="sgBbForm"  >
			    ...
			    <input id="COMPANY_DENGLU_CODE" class="span12" check-type="maxlength" maxlength="36" name="COMPANY_DENGLU_CODE" fieldname="COMPANY_DENGLU_CODE" type="hidden" />
			        <tr>
				...
				<thead>
					<tr>
						...
						<th  fieldname="NAMES" colindex=3 tdalign="center" >&nbsp;岗位&nbsp;</th>
						<th  fieldname="SG_NAME" colindex=4 tdalign="center" >&nbsp;姓名&nbsp;</th>
