		...
		<div class="modal-body" id="contentdivid" style="padding: 1px;">
			<form class="form-horizontal" id="azgzform">
				<input type="hidden" fieldname="JXSB_SYGL_UID"> <input
					type="hidden" fieldname="JXSB_AZGZ_UID"> <input id="SET_GONGCHENG_UID"
					type="hidden" fieldname="GONGCHENG_UID">
			...
			%>
										<a title="选择" href="javascript:;" style="width: 10%" id="sba"
											onclick="checkSb()"><i
			...
			%>
					...
					<div class="widget-body">
					 ...
					 <div class="widget-box">
					<div class="widget-header widget-header-flat">
						<h4 class="widget-title smaller">施工单位审核意见 </h4>
					</div>
					<div class="widget-body">
						<div class="widget-main">
							<div class="form-horizontal">
								...
								<div class="form-group"
									style="margin: 0;line-height: 15px;font-size: 14px;">
									<label style="color: #667E99;padding-left: 0px;" for="inputWarning"
										class="col-xs-12 col-sm-2 control-label no-padding-right">施工总承包单位审核意见</label>
									<div class="col-xs-12 col-sm-10">
										<textarea type="text"  rows="3" style="width: 100%" id="SG_SHENHE_YJ" name="SG_SHENHE_YJ" fieldname="SG_SHENHE_YJ" ></textarea>
									</div>
								</div>
								<div style="margin: 1px;"></div>
								<div class="form-group"
									style="margin: 0;line-height: 15px;font-size: 14px;">
									<label style="color: #667E99;padding-left: 0px;" for="inputWarning" 
										class="col-xs-12 col-sm-2 control-label no-padding-right">总承包指定起重机械专职安全管理人员
</label>
									<div class="col-xs-12 col-sm-4">
										<input id="AQGL_REN" style="width: 90%" type="text"
											  fieldname="AQGL_REN" name="AQGL_REN">
									</div>
								</div>
					...
					<div class="widget-box" style="display: none;" id="shdiv">
					<div class="widget-header widget-header-flat">
						<h4 class="widget-title smaller">审核意见</h4>
					</div>
					<div class="widget-body">
						<div class="widget-main">
							<div class="form-horizontal" id="xmxxform">
								...
								<div class="form-group"
									style="margin: 0;line-height: 15px;font-size: 14px;">
									<label style="color: #667E99;" for="inputWarning"
										class="col-xs-12 col-sm-2 control-label no-padding-right">监理单位审核意见</label>
									<div class="col-xs-12 col-sm-10">
										<textarea type="text" readonly="readonly" rows="3" style="width: 100%" id="JL_SHENHE_YJ" name="JL_SHENHE_YJ" fieldname="JL_SHENHE_YJ" ></textarea>
									</div>
								</div>
								<div class="form-group"
									style="margin: 0;line-height: 15px;font-size: 14px;">
									<label style="color: #667E99;" for="inputWarning"
										class="col-xs-12 col-sm-2 control-label no-padding-right">小组审核意见</label>
									<div class="col-xs-12 col-sm-10">
										<textarea type="text" readonly="readonly" rows="3" style="width: 100%" id="XZSH_YIJIAN" name="XZSH_YIJIAN" fieldname="XZSH_YIJIAN" ></textarea>
									</div>
								</div>
								...
								<div class="form-group"
									style="margin: 0;line-height: 15px;font-size: 14px;">
									<label style="color: #667E99;" for="inputWarning"
										class="col-xs-12 col-sm-2 control-label no-padding-right">告知受理意见/登记部门意见</label>
									<div class="col-xs-12 col-sm-10">
										<textarea type="text" readonly="readonly" rows="3" style="width: 100%" id="GZ_SHOULI_YJ" name="GZ_SHOULI_YJ" fieldname="GZ_SHOULI_YJ" ></textarea>
									</div>
								</div>
...
var validform;
	...
	ace.load_ajax_scripts(scripts, function() {
		...
		if('<%=type%>'=='view'){
			$("#azgzform").find("input,textarea").each(function(){
				   $(this).attr('readonly','readonly');
				});
		}
		$("#SET_GONGCHENG_UID").val($("#GDZXT_XM_ID").val());
	...
	function query(){
		...
		$.ajax({
			...
			success : function(response) {			
				...
				if(resultobj.SHENHE_STATUS=="0"){
					$("#sba").hide();
				}
				$("#SSHENHE_STATUS").val("0");
				return true;
				}
		});		
	...
	function save() { //azgz add  
		var YXQ_BEGIN = document.getElementsByName("SBAZ_DATE")[0].value; 
		var YXQ_END = document.getElementsByName("SBAZ_E_DATE")[0].value; 
		if(YXQ_END<YXQ_BEGIN){
		bootbox.alert("安装日期不能大于截止日期！");
		}else{
	    if(validform.check()){
		bootbox.confirm("请核对信息无误后保存！", function(result) {
			if (result) {
	...
	function queryXmxx() {
		$.ajax({
			url : contextPath + "/sggc/projectsGongchengController/queryXmxx?gcid="+$("#GDZXT_XM_ID").val(),
			data :null,
			cache : false,
			async : false,
			dataType : "json",
