...
var controllername= "${pageContext.request.contextPath }/units/projectsUnitsController.do";
var type ="<%=type%>";
//页面初始化
$(function() {
	init();
	...
	$("#btnQuery").click(function() {
		//生成json串
		var data = combineQuery.getQueryCombineData(queryForm,frmPost,projectsUnitsList);
		//调用ajax插入
		defaultJson.doQueryJsonList(controllername+"?query",data,projectsUnitsList);
	});
	//按钮绑定事件(保存)
	$("#btnSave").click(function() {
		if($("#projectsUnitsForm").validationButton())
		{
		    //生成json串
		    var data = Form2Json.formToJSON(projectsUnitsForm);
		  //组成保存json串格式
		    var data1 = defaultJson.packSaveJson(data);
		  //调用ajax插入
		    if($("#ID").val() == "" || $("#ID").val() == null){
    			defaultJson.doInsertJson(controllername + "?insert", data1,projectsUnitsList);
    			$("#projectsUnitsForm").clearFormResult();
    		}else{
    			defaultJson.doUpdateJson(controllername + "?insert", data1,projectsUnitsList);
    		}
		}else{
			requireFormMsg();
		  	return;
    ...
    $("#btnClear_Bins").click(function() {
        $("#projectsUnitsForm").clearFormResult();
        $("#projectsUnitsForm").cancleSelected();
...
<%--		}--%>
<%--		//表单赋值--%>
<%--		$("#$("#projectsUnitsForm")").setFormValues(tempJson);--%>
<%--	}--%>
}
...
function tr_click(obj){
	...
	$("#projectsUnitsForm").setFormValues(obj);
}
...
function rowView(index){
	var obj = $("#projectsUnitsList").getSelectedRowJsonByIndex(index);
...
<div class="row-fluid">
    ...
    <div class="overFlowX">
	<table class="table-hover table-activeTd B-table" id="projectsUnitsList" width="100%" type="single" pageNum="5">
		<thead>
			<tr>
	...
	<div class="B-small-from-table-autoConcise">
     ...
     <form method="post" id="projectsUnitsForm"  >
      <table class="B-table" width="100%" >
