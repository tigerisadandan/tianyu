...
var controllername= "${pageContext.request.contextPath }/person/sgZizhiController.do";
var type ="<%=type%>";
//页面初始化
$(function() {
	init();
	...
	$("#btnQuery").click(function() {
		//生成json串
		var data = combineQuery.getQueryCombineData(queryForm,frmPost,sgZizhiList);
		//调用ajax插入
		defaultJson.doQueryJsonList(controllername+"?query",data,sgZizhiList);
	});
	//按钮绑定事件(保存)
	$("#btnSave").click(function() {
		if($("#sgZizhiForm").validationButton())
		{
		    //生成json串
		    var data = Form2Json.formToJSON(sgZizhiForm);
		  //组成保存json串格式
		    var data1 = defaultJson.packSaveJson(data);
		  //调用ajax插入
		    if($("#ID").val() == "" || $("#ID").val() == null){
    			defaultJson.doInsertJson(controllername + "?insert", data1,sgZizhiList);
    			$("#sgZizhiForm").clearFormResult();
    		}else{
    			defaultJson.doUpdateJson(controllername + "?insert", data1,sgZizhiList);
    		}
		}else{
			requireFormMsg();
		  	return;
...
<%--		}--%>
<%--		//表单赋值--%>
<%--		$("#$("#sgZizhiForm")").setFormValues(tempJson);--%>
<%--	}--%>
}
...
function rowView(index){
	var obj = $("#sgZizhiList").getSelectedRowJsonByIndex(index);
...
<div class="row-fluid">
    ...
    <div class="overFlowX">
	<table class="table-hover table-activeTd B-table" id="sgZizhiList" width="100%" type="single" pageNum="5">
		<thead>
			<tr>
 			    <th  name="XH" id="_XH" style="width:10px" colindex=1 tdalign="center">&nbsp;#&nbsp;</th>
				<th fieldname="SG_ZIZHI_UID" colindex=0 tdalign="center" >&nbsp;施工项目工程类型时对应的专业&nbsp;</th>
				<th fieldname="ZIZHI_NAME" colindex=1 tdalign="center" maxlength="30" >&nbsp;&nbsp;</th>
				<th fieldname="ZIZHI_TYPE" colindex=2 tdalign="center" >&nbsp;资质类型，ZB－施工总承包资质；CB－专业承包资质；FB－劳务分包资质&nbsp;</th>
				<th fieldname="ZHUANYE_NAME" colindex=3 tdalign="center" maxlength="30" >&nbsp;专业名称&nbsp;</th>
				<th fieldname="SERIAL_NO" colindex=4 tdalign="center" >&nbsp;排序号&nbsp;</th>
             </tr>
		</thead>
		<tbody>
           </tbody>
	...
	<div class="B-small-from-table-autoConcise">
     ...
     <form method="post" id="sgZizhiForm"  >
      <table class="B-table" width="100%" >
