...
function rowView1(t){	
	...
	var status=tempJson.STATUS;
	 ...
	 if(status==30){
			  ...
			  $.ajax({
					url : controllername+"updateCopyPerson?t_id="+t_id+"&u_id="+u_id,
					data : {},  
					cache : false,
					async :	false,
					dataType : "json",  
					...
					$("#resultXML").val(response.msg);
						id=response.msg;
						//在进入add页面前，把数据进行复制。 
		                  window.open("${pageContext.request.contextPath }/jlpersonadd/"+id+"/"+t_id,"人员信息查看");//传递一个ID给详细页面，用于查找   
					}
				});
          }else{
             window.open("${pageContext.request.contextPath }/jlpersonadd/"+tempJson.PERSON_LIBRARY_UID,"人员信息查看");//传递一个ID给详细页面，用于查找 
          }
	            ...
	            <table width="100%" class="table-hover table-activeTd B-table" id="DT1" type="single"  pageNum="18">
	              <thead>
	                <tr>
	                		...
	                		<th fieldname="JL_PERSON_UID" id="doSelect" name="doSelect" tdalign="center" colindex=2 CustomFunction="doMoreChoose" >
		                		<label class="checkbox inline" >
		                        <input type="checkbox" id="allSelect" onclick="doAllSelect(this)" name="allSelect"/>
		                        </label>
	                        </th>
	                		<th rowspan="2" fieldname="JL_PERSON_UID" tdalign="center" colindex=2 CustomFunction="doRandering" noprint="true">&nbsp;&nbsp;</th>
	                        <th fieldname="PERSON_NAME" width="15%" colindex=0 tdalign="center" >&nbsp;姓名&nbsp;</th>
							<th fieldname="COMPANY_NAME" width="30%" colindex=1 tdalign="center" maxlength="30" CustomFunction="doCn">&nbsp;监理单位名称&nbsp;</th>
							<th fieldname="DENGLU_CODE" width="20%" colindex=1 tdalign="center" maxlength="30" hidden>&nbsp;监理单位登录code&nbsp;</th>
							<th fieldname="SEX" width="4%" colindex=3  tdalign="center" >&nbsp;性别&nbsp;</th>
							<th fieldname="SHENFENZHENG" width="20%" colindex=5 tdalign="center" maxlength="30">&nbsp;身份证&nbsp;</th>
							<!--  
