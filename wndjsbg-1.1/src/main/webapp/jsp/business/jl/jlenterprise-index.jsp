...
function rowView(obj){
	...
	$("#resultXML").val($("#DT1").getSelectedRow());
	$(window).manhuaDialog({"title":"监理企业信息库>查看","type":"text","content":"${pageContext.request.contextPath }/jsp/business/jl/jlenterprise-add.jsp?type=detail","modal":"1"});
}
...
function doeditor(obj){
	...
	if(start=='30'){
		rehtml+="<a href='javascript:void(0)' onclick='editorU("+obj.ENTERPRISE_LIBRARY_UID+")'  title='审核'><i class='icon-file showXmxxkInfo'></i></a>";
	}else{
...
function copyCompanyInfo(elUid){
	$.ajax({
		url : controllername+"/updateCopyByStatus",
		data : {"uid":elUid,"status":15,"reason":""},
		cache : false,
		async :	false,
		dataType : "json",  
		type : 'post',
		success : function(response) {
			...
			window.open("${pageContext.request.contextPath }/jlentedit/"+elUid+"/"+uid,"企业信息审批");
		}
	})
}
				...
				<table class="B-table" width="100%">
					...
					<TR style="display: none;">
						<TD class="right-border bottom-border">
							...
							<INPUT type="text" class="span12" kind="text" id="ZT_ID" fieldname="STATUS" value="30" operation="="/>
					...
					<tr>
						...
						<nobr>	
     						 <input type=radio value="10" name="status" onclick="javascript:checkStatus(10)">已通过&nbsp;&nbsp;
     						  <input type=radio id="zt2"  value="20" name="status" onclick="javascript:checkStatus(20)">未通过&nbsp;&nbsp;
      						 <input type=radio id="zt1"  value="30" name="status" onclick="javascript:checkStatus(30)" checked="checked"  >待审核&nbsp;&nbsp;
	            ...
	            <table width="100%" class="table-hover table-activeTd B-table" id="DT1" type="single"  pageNum="18">
	                <thead>
	                	<tr>
							...
							<th fieldname="COMPANY_NAME" colindex=5 tdalign="left" style="width:250px">&nbsp;企业全称&nbsp;</th>
							...
							<th fieldname="STATUS" colindex=11 tdalign="center" style="width:5px">&nbsp;状态&nbsp;</th>
	                	</tr>
	                </thead>
	              	<tbody></tbody>
	           </table>
