...
$(function() {
	...
	$("#btnSave").click(function() {
		if($("#yxClEnterpriseForm").validationButton()){
			xConfirm("信息确认","确认提交审核结果？");
			$('#ConfirmYesButton').attr('click','');
			$('#ConfirmYesButton').one("click",function(){
			    //生成json串
			    var data = Form2Json.formToJSON(yxClEnterpriseForm);
			  //组成保存json串格式
			    var data1 = defaultJson.packSaveJson(data);
			  //调用ajax插入
			    if($("#CL_ENTERPRISE_UID").val()!= ""&&$("#CL_ENTERPRISE_UID").val()!= null){
...
function init(){
	if(type == "insert"){
		...
		$("#QCL_ENTERPRISE_UID").val(tempJson.CL_ENTERPRISE_UID);
		//查询表单赋值
		...
		$.ajax({
			...
			success : function(response) {				
				...
				$("#yxClEnterpriseForm").setFormValues(resultobj);	
				getClGcalxx(resultobj.CL_ENTERPRISE_UID);//加载工程案例信息
		...
		//列出审批意见信息
		var queryconditionXX = "{\"logic\":\"and\",\"fieldname\":\"CL_ENTERPRISE_UID\",\"operation\":\"=\",\"value\":\""+tempJson.CL_ENTERPRISE_UID+"\",\"fieldtype\":'',\"fieldformat\":''}";
		...
		//加载附件
		queryFileData(tempJson.CL_ENTERPRISE_UID, "", "", "777003");
	}
}
...
function getClGcalxx(cluid){
	var queryconditionXX = "{\"logic\":\"and\",\"fieldname\":\"CL_ENTERPRISE_UID\",\"operation\":\"=\",\"value\":\""+cluid+"\",\"fieldtype\":'',\"fieldformat\":''}";
	...
	$.ajax({
		url :"${pageContext.request.contextPath }/qyxx/yxClEnterpriseGcalxxController?query",
		data : data,
		cache : false,
		async :	false,
		dataType : "json",  
...
<div class="row-fluid">
	<div class="B-small-from-table-autoConcise">
     ...
     <form method="post" id="yxClEnterpriseForm"  >
           ...
           <tr>
				<th width="8%" class="right-border bottom-border text-right">组织机构代码</th>
	       	 	...
	       	 	<td class="bottom-border right-border" width="15%">
						...
						<tbody fjlb="7781" class="files showFileTab" data-toggle="modal-gallery" data-target="#modal-gallery" onlyView="true"></tbody>
					</table>
	       	 	</td>        	
           </tr>
           <tr>
            ...
            <tr>
				<th width="8%" class="right-border bottom-border text-right">营业有效期</th>
	       	 	...
	       	 	<td class="bottom-border right-border" width="15%">
						...
						<tbody fjlb="7782"  class="files showFileTab" data-toggle="modal-gallery" data-target="#modal-gallery" onlyView="true">
						</tbody>
					</table>
	       	 	</td>        	
           </tr>
         ...
         <tr>
          ...
          <tr>
						...
						<tr id="gcalxxTr{0}" >
							...
							<td >
								<table role="presentation" id="fjtable{0}" class="table table-striped">
									<tbody id="FJTBODYID{0}" glid1="{0}" fjlb="7780"  class="files showFileTab" data-toggle="modal-gallery" data-target="#modal-gallery"></tbody>
								</table>
							</td>
						</tr>
					</textarea>
			...
			<div id="shjltxdivid">
				...
				<h3 style="color:white;">材料设备企业资质审核
				<span class="pull-right">
  ...
  <div align="center">
 	<FORM name="frmPost" method="post" style="display:none" target="_blank">
         ...
         <input type="hidden" name="txtFilter"  order="desc" fieldname = "CL_ENTERPRISE_UID" id = "txtFilter">
