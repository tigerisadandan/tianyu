...
function doeditor(obj){
	var rehtml=" ";
	var start=obj.ISCQ;
	var xypjuid=obj.XYPJ_UID;

	if(start==1){
		rehtml+="<a href='javascript:void(0)' onclick='editorU(this)'  title='抽签结果'><i class='icon-file showXmxxkInfo'></i></a>";
	}
	if(xypjuid!=null&&xypjuid!=''){
		rehtml+="&nbsp;<a href='javascript:void(0)' onclick='viewPjxx(this)'  title='查看评价信息'><i class='icon-file showXmxxkInfo'></i></a>";
	}
	rehtml+="&nbsp;<a href='javascript:void(0)' onclick='rowView(this)'  title='查看信息'><i class='icon-file showXmxxkInfo'></i></a>";
	return rehtml;
}

//查看中标企业评价信息
function viewPjxx(obj){
	while(obj.tagName.toLowerCase() != "tr"){
		obj = obj.parentNode;
		if(obj.tagName.toLowerCase() == "table")return null;
	}
	...
	$("#resultXML").val($("#DT1").getSelectedRow());
	$(window).manhuaDialog({"title":"微型工程>中标承包商评价","type":"text","content":"${pageContext.request.contextPath }/jsp/business/wxgc/yxwxgc-xypj.jsp?type=detail","modal":"1"});
}
...
function editorU(obj){
	...
	$("#resultXML").val($("#DT1").getSelectedRow());
	$(window).manhuaDialog({"title":"微型工程>抽签结果","type":"text","content":"${pageContext.request.contextPath }/jsp/business/wxgc/yxwxgc-cqjg.jsp?type=detail","modal":"1"});
}

function iscqfun(obj){
	var iscq;
	var zbr=obj.ZB_YXCBS_UID;
	if(zbr==''||zbr==null){
		iscq="未抽签";
	}else{
		iscq="已抽签";
		...
		<div class="B-small-from-table-autoConcise">
			<form method="post" id="queryForm">
				<table class="B-table" width="100%">
					...
					<input  type="hidden" id="QTSHZT" name="ZT" value='10' fieldname="ZT" operation="="  >
					<input type="hidden" kind="text" id="ISCQ_UID" fieldname="ISCQ" value="0" operation="="/>
					<!--可以再此处加入hidden域作为过滤条件 -->
					<tr>
						<th  width="5%" class="right-border bottom-border text-right">抽签状态</th>
						<td class="right-border bottom-border" width="18%">
      						 ...
      						 <input type=radio id="zt1"  value="1" name="status" onclick="checkStatus(this.value)"  >已抽签&nbsp;&nbsp;
      						 <input type=radio id="zt2"  value="0" name="status" onclick="checkStatus(this.value)"  checked="checked"  >未抽签
						</td>
						...
						<th width="5%" class="right-border bottom-border text-right">工程编号</th>
						<td class="right-border bottom-border" width="10%">
							<input class="span12" type="text" id="QGC_CODE" name="GC_CODE" fieldname="GC_CODE" operation="like" >
						</td>
			            ...
			            <td class="text-left bottom-border text-right" width="30%">
	            ...
	            <table width="100%" class="table-hover table-activeTd B-table" id="DT1" type="single"  pageNum="18">
	                <thead>
	                	<tr>
	                		...
	                		<th fieldname="WXGC_UID" colindex=2 tdalign="center" style="width:10px"  CustomFunction="doeditor" noprint="true">&nbsp;&nbsp;</th>
<%--	                		<th fieldname="ISCQ" colindex=4 tdalign="center" style="width:5px"  CustomFunction="iscqfun" noprint="false" >&nbsp;是否抽签&nbsp;</th>--%>
							<th fieldname="GC_NAME" colindex=4 tdalign="left" style="width:250px"   >&nbsp;工程名称&nbsp;</th>
							<th fieldname="GC_CODE" colindex=3  style="width:5px"  tdalign="center" >&nbsp;工程编号&nbsp;</th>
							<th fieldname="GC_TYPE_NAME" colindex=5  style="width:5px"   tdalign="center" >&nbsp;工程类型&nbsp;</th>
							...
							<th fieldname="QY_NAME" colindex=9 tdalign="center" style="width:20px" >&nbsp;招标人&nbsp;</th>
<%--							<th fieldname="JHGQ" colindex=12 tdalign="center" style="width:3px">&nbsp;计划工期(天)&nbsp;</th>--%>
							<th fieldname="ZBJ_SV" colindex=13 tdalign="right" style="width:5px">&nbsp;明标价(元)&nbsp;</th>
