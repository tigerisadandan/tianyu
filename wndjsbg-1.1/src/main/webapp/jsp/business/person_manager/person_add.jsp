...
$(function() {
 ...
 $("#btnFindOne").click(function() {
	 var jg = $("input:radio[name='SPXZZT']:checked").val();
		//if(check()){
			if(jg=="10"){
				if(isName($("#PERSON_NAME"))&&isCardNo($("#SHENFENZHENG"))&&isPhone($("#PHONE"))&&isEmail($("#EMAIL"))&&beDate($("#HTBEGIN_DATE"))&&enDate($("#HTEND_DATE"))&&fujianNotNull()&&zhichengNotNull()&&doZsFj())
                   {
                      pass();
				   }
				}
...
function pass(){
	...
	var flag = true;
	  ...
	  //对证书为建造师证，证书编号不能为空的设置 
	    $("#zhigeList tr").each(function(){
	    	//var code=$(item).parent().parent().find("#ZHENGSHU_CODE");
			if($(this).find("#ZHENGSHU_NAME")!=null){
				 if($(this).find("#ZHENGSHU_NAME").val()=="10"||$(this).find("#ZHENGSHU_NAME").val()=="11"){
					if($(this).find("#ZHENGSHU_CODE").val()=="0"||$(this).find("#ZHENGSHU_CODE").val()==""){
...
function setFormValues(){
	...
	$.ajax({
		...
		$("#QID1").val(resultobj.SG_PERSON_UID);
		$("#QID2").val(resultobj.SG_PERSON_UID);
		}   
    });
...
function doFileChayi(nowObj,oldObj){
	$.ajax({
		url : "${pageContext.request.contextPath }/fileUploadController.do?doFileHasChange&uid="+nowObj.SG_PERSON_LIBRARY_UID+"&oldUid="+oldObj.SG_PERSON_LIBRARY_UID+"&type=SG_PERSON_LIBRARY",
		data : {},
		cache : false,
		async :	false,
		dataType : "json",  
	...
	$.ajax({
		...
		success : function(response) {
			...
			$(obj1).each(function(){
				...
				$(cloneNew).find("#ZSBEGIN_DATE").val(this.BEGIN_DATE.substring(0,10));
				$(cloneNew).find("#ZSEND_DATE").val(this.END_DATE.substring(0,10));
    ...
    <div class="B-small-from-table-autoConcise">
     <form method="post" id="queryForm"  >
      ...
      <!--可以再此处加入hidden域作为过滤条件 -->
      	<TR style="display:none;">
	        <TD class="right-border bottom-border">
	        	<INPUT type="text" class="span12" kind="text" id="QID2" name="ID"  fieldname="a.SG_PERSON_UID" value="" operation="="/>
	...
	<div class="B-small-from-table-autoConcise">
      ...
      <form method="post" id="sgPersonLibraryForm">
		<%--<div class="tab-content"> --%>
				...
				提交前经过修改的人员信息的内容会以<i class='icon-warning-sign showXmxxkInfo' title='"+title+"'></i>提示,您也可以<span id="hideTs"><a href="javascript:void(0)" onclick="hideTs(true)">隐藏</a></span><span id="showTs"><a href="javascript:void(0)" onclick="hideTs(false)">显示</a></span>该提示
				<table class="B-table" width="100%" id="val1">
			        ...
			        <input type="hidden" id="SG_PERSON_LIBRARY_UID" fieldname="SG_PERSON_LIBRARY_UID" name="SG_PERSON_LIBRARY_UID"/>
				  	...
				  	<input type="hidden" id="COMPANY_DENGLU_CODE" fieldname="COMPANY_DENGLU_CODE" name = "COMPANY_DENGLU_CODE"/>
						...
						<div>
						      <span class="btn btn-fileUpload" id="addFileSpan" onclick="doSelectFile(this,'parent');" ywid="SG_PERSON_LIBRARY_UID" target_type="2" file_type="16" business_type="SG_PERSON_LIBRARY">
								...
								<table role="presentation" class="table table-striped">
									<tbody id="SHENFENZHENGHAO" fjlb="0701" class="files showFileTab" data-toggle="modal-gallery" data-target="#modal-gallery"></tbody>
								</table>
							</div>
						</td>
					</tr>
					...
					<tr>	
						...
						<td  class="right-border bottom-border">
							<div >
								...
								<table role="presentation" class="table table-striped ">
									<tbody id="ZHICHENG" fjlb="0701" class="files showFileTab" data-toggle="modal-gallery" data-target="#modal-gallery"></tbody>
								</table>
							</div>
						</td>
					</tr>	
					...
					<tr>  
					   ...
					   <td>
						 <div>
							...
							<table role="presentation" class="table table-striped">
								<tbody id="LAODONGHT" fjlb="0701" class="files showFileTab" data-toggle="modal-gallery" data-target="#modal-gallery"></tbody>
							</table>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td>  
						    <div>
								...
								<table role="presentation" class="table table-striped">
									<tbody id="SHEBAOZM" fjlb="0701" class="files showFileTab" data-toggle="modal-gallery" data-target="#modal-gallery"></tbody>
								</table>
							</div>
						</td>
					</tr>
				...
				<table  width="100%" class="table-hover table-activeTd B-table" id="RyList" type="single" pageNum="1000" >
				 <tr>
			            ...
			            <td class="right-border bottom-border" >&nbsp;
				            <input class="span12" id="SPXZZT" type="radio" placeholder="" kind="dic" src="SPXZZT" operation="=" name="SPXZZT" onchange="clickRadioShowDate()" fieldname="SHENHE_JIEGUO">&nbsp;&nbsp;   <b><span id="jgtext" style="color:red;font-size: 14px"></span></b>
				        </td>
			    </tr>
				<tr>
