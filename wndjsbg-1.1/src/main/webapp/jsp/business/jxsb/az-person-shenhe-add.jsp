...
$(function() {
	...
	$("#btnQuery").click(function() {
		//生成json串
		var data = combineQuery.getQueryCombineData(queryForm,frmPost,azPersonShenheList);
		//调用ajax插入
		defaultJson.doQueryJsonList(controllername+"?query",data,azPersonShenheList);
	});
	//按钮绑定事件(保存)
	$("#btnSave").click(function() {
		if($("#azPersonShenheForm").validationButton())
		{
		    //生成json串
		    var data = Form2Json.formToJSON(azPersonShenheForm);
		  //组成保存json串格式
		    var data1 = defaultJson.packSaveJson(data);
		  //调用ajax插入
		    if($("#ID").val() == "" || $("#ID").val() == null){
    			defaultJson.doInsertJson(controllername + "?insert", data1,azPersonShenheList);
    			$("#azPersonShenheForm").clearFormResult();
    		}else{
    			defaultJson.doUpdateJson(controllername + "?insert", data1,azPersonShenheList);
    		}
		}else{
			requireFormMsg();
		  	return;
    ...
    $("#btnClear_Bins").click(function() {
        $("#azPersonShenheForm").clearFormResult();
        $("#azPersonShenheForm").cancleSelected();
...
<%--		}--%>
<%--		//表单赋值--%>
<%--		$("#$("#azPersonShenheForm")").setFormValues(tempJson);--%>
<%--	}--%>
}
...
function tr_click(obj){
	...
	$("#azPersonShenheForm").setFormValues(obj);
}
...
function rowView(index){
	var obj = $("#azPersonShenheList").getSelectedRowJsonByIndex(index);
...
<div class="row-fluid">
    ...
    <div class="overFlowX">
	<table class="table-hover table-activeTd B-table" id="azPersonShenheList" width="100%" type="single" pageNum="5">
		<thead>
			<tr>
	...
	<div class="B-small-from-table-autoConcise">
     ...
     <form method="post" id="azPersonShenheForm"  >
      <table class="B-table" width="100%" >
