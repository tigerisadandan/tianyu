...
/**	$("#btnExpExcel").click(function() {
    ...
    $("#btnClear").click(function() {
     ...
     $("#yqQuery").click(function() { //延期申请
     $(window).manhuaDialog({"title":"延期申请>审核","type":"text","content":"${pageContext.request.contextPath }/jsp/business/dtgl/azqy/azqy-companys-index-yqsq.jsp","modal":"2"});
       //window.open("${pageContext.request.contextPath }/jsp/business/dtgl/azqy/azqy-companys-index-yqsq.jsp?id=42",'延期申请');
    });
    
     $("#dqQuery").click(function() { //到期
     $(window).manhuaDialog({"title":"延期申请>审核","type":"text","content":"${pageContext.request.contextPath }/jsp/business/dtgl/azqy/azqy-companys-index-dq.jsp","modal":"2"});
       //window.open("${pageContext.request.contextPath }/jsp/business/dtgl/azqy/azqy-companys-index-yqsq.jsp?id=42",'延期申请');
    });
...
function rowView(obj){
	...
	$("#resultXML").val(data);
	$(window).manhuaDialog({"title":"建设单位>详细信息","type":"text","content":"${pageContext.request.contextPath }/jsp/business/dtgl/azqy/az-company-add-dq.jsp?type=detail","modal":"2"});
}
...
function getDqcount(){	
	$.ajax({
		url : controllername+"?queryCount&type=dq",
	//	data : data1,
		cache : false,
		async : false,
		dataType : "json",
		...
		success : function(response) {
			...
			var num=resultobj.COUNT;
			$("#dq").html("有"+num+"个");
		}
	});
}

function getYqsqcount(){	
	$.ajax({
		url : controllername+"?queryCount&type=yqsq",
	//	data : data1,
		cache : false,
		async : false,
		dataType : "json",
			...
			<form method="post" id="queryForm">
				<table class="B-table" width="100%">
					...
					<tr>
						...
						<th width="5%" class="right-border bottom-border text-right">登陆代码</th>
						<td class="right-border bottom-border" width="20%">
							<input class="span12" type="text" onkeydown="enterSumbit()" id="QCOMPANY_NAME" name="COMPANY_NAME" fieldname="DL_CODE" operation="=" >
						</td>
						<td class="right-border bottom-border" width="30%">
							<button id="dqQuery" class="btn btn-link"  type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;"><span id="dq"></span>安全许可证到期</button>
           					<button id="yqQuery" class="btn btn-link" type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;"><span id="yqsq"></span>申请延期</button>
						</td>				
	            ...
	            <table width="100%" class="table-hover table-activeTd B-table" id="DT1" type="single"  pageNum="18">
	                <thead>
	                	<tr>
							...
							<th fieldname="COMPANY_NAME" colindex=2 tdalign="center" maxlength="30" CustomFunction="nameFun">&nbsp;企业名称&nbsp;</th>
							<th fieldname="ENABLED" colindex=2 tdalign="center" maxlength="30" hidden="" ></th>
