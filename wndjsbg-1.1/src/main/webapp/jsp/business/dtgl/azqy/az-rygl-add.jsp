...
var controllernamery= "${pageContext.request.contextPath }/jxsb/azPersonShenheController";
var type ="<%=type%>";
//页面初始化
$(function() {
	init();
 var input = $("#dis").find("input:radio");
   input.attr("disabled","disabled");
	...
	msg="确认审核通过？";
		  ...
		  //调用ajax插入
		    if($("#AZ_PERSON_SHENHE_UID").val() == "" || $("#AZ_PERSON_SHENHE_UID").val() == null){
    			...
    			if($("input:radio[name='SHENHE_JIEGUO']:checked").val()==2){
    				 ...
    				 if(reason.length==0){
 						alert("请填写审批不通过理由!");
 						return;
 						}else{defaultJson.doUpdateJson(controllernamery + "?update", data1);}
    			}else{
...
//	defaultJson.doQueryJsonList(controllername+"?query",data);
	...
	if (type == "insert") {
		...
		$("#QAZ_PERSON_SHENHE_UID").val(tempJson.AZ_PERSON_SHENHE_UID);
	...
	<div class="B-small-from-table-autoConcise">
      <h4 class="title">安装人员
       ...
       <form method="post" id="personQueryForm"  >
     		<input type="hidden" id="QAZ_PERSON_SHENHE_UID" fieldname="AZ_PERSON_SHENHE_UID" name="AZ_PERSON_SHENHE_UID" operation="=" />
      </form>
      <div style="height:5px;"></div>
     <form method="post" id="personForm"  >
     	<table class="B-table" width="100%" >
      		<input type="hidden" id="AZ_PERSON_SHENHE_UID" fieldname="AZ_PERSON_SHENHE_UID" name = "AZ_PERSON_SHENHE_UID"/>
      		<tr>
