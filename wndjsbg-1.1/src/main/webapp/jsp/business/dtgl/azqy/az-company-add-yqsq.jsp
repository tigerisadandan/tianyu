...
$(function() {
   ...
   input.attr("disabled","disabled");
	...
	msg="确认审核通过？";
		  ...
		  //调用ajax插入
		    if($("#AZ_COMPANY_UID").val() == "" || $("#AZ_COMPANY_UID").val() == null){
    			...
    			if($("input:radio[name='SHENHE_JIEGUO']:checked").val()==2){
    				 var reason = $("#YQ_DESC").val();
    				 if(reason.length==0){
 						alert("请填写审批不通过理由!");
 						return;
 						}else{defaultJson.doUpdateJson(controllername + "?updateYqsq", data1);}
    			}else{
    			defaultJson.doUpdateJson(controllername + "?updateYqsq", data1);
    			}
    		}
	...
	$("#queryPerson").click(function() {
           $(window).manhuaDialog({"title":"企业信息>安装人员信息","type":"text","content":"${pageContext.request.contextPath }/jsp/business/dtgl/azqy/azqy-azryk-index.jsp?qyid="+$("#AZ_COMPANY_UID").val(),"modal":"2"});
    });
...
<div class="row-fluid">
	 <form method="post" id="queryForm"  >
      ...
      <form method="post" id="stopForm">
      	<input type="hidden" id="stop_AZ_COMPANY_UID" fieldname="AZ_COMPANY_UID" name = "AZ_COMPANY_UID"/>
      	<input type="hidden" id="ENABLED" fieldname="ENABLED" name = "ENABLED" value="0"/>  
      </form>
	...
	<div class="B-small-from-table-autoConcise">
      <h4 class="title"> 安装单位信息  
      	...
      	<span class="pull-right">
      		...
      		<button id="queryPerson" class="btn" type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;">查看人员信息</button>
      		<button id="stopQy"  class="btn" type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;">停用</button>
	        ...
	        <tr>
	         ...
	         <tr>
				<th width="15%" class="right-border bottom-border text-right">安全生产许可证编号</th>
	       	 	...
	       	 	<td colspan="3" class="bottom-border right-border">
					<input style="width:40%" id="YXQ_BEGIN" type="text" readonly="readonly" fieldname="YXQ_BEGIN" name = "YXQ_BEGIN"  />到
					<input style="width:40%" id="YXQ_END" type="text" readonly="readonly" fieldname="YXQ_END" name = "YXQ_END"  />
				</td>
	        </tr>
	         <tr>
    ...
    <div class="B-small-from-table-autoConcise">
      ...
      <div style="height:5px;"></div>
     	<table class="B-table" width="100%" >
      		...
      		<input type="hidden" id="YQ_YQ_STATUS" fieldname="YQ_STATUS" name = "YQ_STATUS" />
			...
			<tr>
				<th width="15%" class="right-border bottom-border text-right">延期日期</th>
	       	 	<td class="bottom-border right-border" style="width:32%">
	       	 	<input class="span12" style="width:45%" id="YQ_B_DATE" type="text" readonly="readonly"  fieldname="YQ_B_DATE" name = "YQ_B_DATE"  />~	
	       	 	  <input style="width:45%" id="YQ_E_DATE" type="text" readonly="readonly"  name = "YQ_E_DATE" fieldname="YQ_E_DATE" />         	        	
	       	 	</td>
				</td>
	        </tr>
			<tr>
				<th width="15%" class="right-border bottom-border text-right">审核详细意见</th>
	       	 	<td class="bottom-border right-border" colspan="3">
	         		<textarea class="span12" rows="2" id="YQ_DESC" check-type="required" maxlength="4000"  fieldname="YQ_DESC" name="YQ_DESC"></textarea>
	       	 	</td>
	        </tr>
