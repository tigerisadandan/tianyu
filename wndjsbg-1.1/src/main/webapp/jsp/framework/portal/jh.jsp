...
function queryTcjh(str){
	$(window).manhuaDialog({"title":"统筹计划>项目查询","type":"text","content":"${pageContext.request.contextPath}/jsp/framework/portal/tcjh_list_query.jsp?str="+str});
}
...
<div class="bzgk-table">
	...
	<tbody>
        ...
        sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE JHID = T.GC_JH_ZT_ID AND ND ='"+year+"' AND XMBS = '0' AND SFYX = '1') AS ZS,");
        sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE JHID = T.GC_JH_ZT_ID AND XMSX = '2' AND ND = '"+year+"' AND XMBS = '0' AND SFYX = '1') AS YJ,");
        sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE ISBT = '1' AND T1.JHID = T.GC_JH_ZT_ID AND GC_TCJH_XMXDK_ID = T1.XMID AND T1.ND ='"+year+"' AND t1.SFYX = '1') AS BT,");
        sbSql.append("(SELECT COUNT(DISTINCT XMID) FROM GC_JH_BGXM WHERE T.GC_JH_ZT_ID = JHID AND SFXG = '1' AND T.ND ='"+year+"' AND SFYX = '1') AS TZ,");
        for(int i = 0; i < xmglgsCoount; i++){
        	xmglgsId = xmglgsValue[i][0].toString();
        	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE XMGLGS ='"+xmglgsId+"' AND GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE JHID = T.GC_JH_ZT_ID AND ND ='"+year+"' AND SFYX = '1')),");
      ...
      <tr>
        ...
        <%
    		sbSql = new StringBuffer();
   	        ...
   	        sbSql.append("WHERE ");
   	        sbSql.append("T.KGSJ IS NOT NULL AND T.WGSJ IS NOT NULL ");
        ...
        <%
			...
			sbSql.append("FROM GC_JH_SJ T ");
			sbSql.append("WHERE ");
			sbSql.append("T.KGSJ IS NULL AND T.WGSJ IS NULL ");
...
<script type="text/javascript" charset="utf-8">
	if(<%=Integer.parseInt(ybz)%> == 0){
        ...
        <%
      	    ...
      	    sbSql.append("FROM GC_JH_SJ T ");
      	    sbSql.append("WHERE ");
      	    sbSql.append("T.KGSJ_SJ IS NOT NULL AND T.WGSJ_SJ IS NOT NULL ");
      	    sbSql.append("AND ((T.ISKYPF = '1' AND T.KYPF_SJ IS NOT NULL) OR (T.ISKYPF = '0')) ");
	       	...
	       	sbSql.append("AND T.ND = '"+year+"' AND T.SFYX = '1' AND T.XMBS = '1' OR (T.ISNOBDXM='1' AND T.XMBS!='1')");
       		sql = sbSql.toString();
		...
		%>
         ...
         <%
         	...
         	sbSql = new StringBuffer();
	   	    ...
	   	    sbSql.append("WHERE ");
	   	 	sbSql.append("T.ND = '"+year+"' AND T.SFYX = '1' AND T.XMBS='1' OR (T.ISNOBDXM='1' AND T.XMBS!='1') ");
    		sql = sbSql.toString();
    		String[][] bdzsArray = DBUtil.query(conn, sql);
    		if(Pub.emptyArray(bdzsArray)){
    			bdzs = String.valueOf(Integer.parseInt(bdzsArray[0][0].toString())*17);
    		}
        ...
        <%
			...
			sbSql.append("FROM GC_JH_SJ T ");
			sbSql.append("WHERE ");
			sbSql.append("((T.KGSJ IS NOT NULL AND T.KGSJ_SJ IS NOT NULL) OR (SYSDATE <=T.KGSJ)) ");
			sbSql.append("AND ((T.WGSJ IS NOT NULL AND T.WGSJ_SJ IS NOT NULL) OR (SYSDATE <=T.WGSJ)) ");
			sbSql.append("AND (((T.ISKYPF = '1' AND T.KYPF IS NOT NULL AND T.KYPF_SJ IS NOT NULL) OR (SYSDATE <=T.KYPF)) OR (T.ISKYPF = '0')) ");
			sbSql.append("AND (((T.ISHPJDS = '1' AND T.HPJDS IS NOT NULL AND T.HPJDS_SJ IS NOT NULL) OR (SYSDATE <=T.HPJDS)) OR (T.ISHPJDS = '0')) ");
			sbSql.append("AND (((T.ISGCXKZ = '1' AND T.GCXKZ IS NOT NULL AND T.GCXKZ_SJ IS NOT NULL) OR (SYSDATE <=T.GCXKZ)) OR (T.ISGCXKZ = '0')) ");
			sbSql.append("AND (((T.ISSGXK = '1' AND T.SGXK IS NOT NULL AND T.SGXK_SJ IS NOT NULL) OR (SYSDATE <=T.SGXK)) OR (T.ISSGXK = '0')) ");
			sbSql.append("AND (((T.ISCBSJPF = '1' AND T.CBSJPF IS NOT NULL AND T.CBSJPF_SJ IS NOT NULL) OR (SYSDATE <=T.CBSJPF)) OR (T.ISCBSJPF = '0')) ");
			sbSql.append("AND (((T.ISCQT = '1' AND T.CQT IS NOT NULL AND T.CQT_SJ IS NOT NULL) OR (SYSDATE <=T.CQT)) OR (T.ISCQT = '0')) ");
			sbSql.append("AND (((T.ISPQT = '1' AND T.PQT IS NOT NULL AND T.PQT_SJ IS NOT NULL) OR (SYSDATE <=T.PQT)) OR (T.ISPQT = '0')) ");
			sbSql.append("AND (((T.ISSGT = '1' AND T.SGT IS NOT NULL AND T.SGT_SJ IS NOT NULL) OR (SYSDATE <=T.SGT)) OR (T.ISSGT = '0')) ");
			sbSql.append("AND (((T.ISTBJ = '1' AND T.TBJ IS NOT NULL AND T.TBJ_SJ IS NOT NULL) OR (SYSDATE <=T.TBJ)) OR (T.ISTBJ = '0')) ");
			sbSql.append("AND (((T.ISCS = '1' AND T.CS IS NOT NULL AND T.CS_SJ IS NOT NULL) OR (SYSDATE <=T.CS)) OR (T.ISCS = '0')) ");
			sbSql.append("AND (((T.ISJLDW = '1' AND T.JLDW IS NOT NULL AND T.JLDW_SJ IS NOT NULL) OR (SYSDATE <=T.JLDW)) OR (T.ISJLDW = '0')) ");
			sbSql.append("AND (((T.ISSGDW = '1' AND T.SGDW IS NOT NULL AND T.SGDW_SJ IS NOT NULL) OR (SYSDATE <=T.SGDW)) OR (T.ISSGDW = '0')) ");
			sbSql.append("AND (((T.ISZC = '1' AND T.ZC IS NOT NULL AND T.ZC_SJ IS NOT NULL) OR (SYSDATE <=T.ZC)) OR (T.ISZC = '0')) ");
			sbSql.append("AND (((T.ISPQ = '1' AND T.PQ IS NOT NULL AND T.PQ_SJ IS NOT NULL) OR (SYSDATE <=T.PQ)) OR (T.ISPQ = '0')) ");
			sbSql.append("AND (((T.ISJG = '1' AND T.JG IS NOT NULL AND T.JG_SJ IS NOT NULL) OR (SYSDATE <=T.JG)) OR (T.ISJG = '0'))");
			sbSql.append("AND T.ND = '"+year+"' AND T.SFYX = '1' AND T.XMBS = '1' OR (T.ISNOBDXM='1' AND T.XMBS!='1')");
			sql = sbSql.toString();
        ...
        <%
			...
			sbSql.append("FROM GC_JH_SJ T ");
			sbSql.append("WHERE ");
			sbSql.append("((T.KGSJ IS NOT NULL AND T.KGSJ_SJ IS NOT NULL) AND (T.KGSJ = T.KGSJ_SJ)) ");
			sbSql.append("OR ((T.WGSJ IS NOT NULL AND T.WGSJ_SJ IS NOT NULL) AND (T.WGSJ = T.WGSJ_SJ)) ");
			sbSql.append("OR (((T.ISZC = '1' AND T.ZC IS NOT NULL AND T.ZC_SJ IS NOT NULL) AND (T.ZC = T.ZC_SJ)) OR (T.ISZC = '0')) ");
			sbSql.append("OR (((T.ISPQ = '1' AND T.PQ IS NOT NULL AND T.PQ_SJ IS NOT NULL) AND (T.PQ = T.PQ_SJ)) OR (T.ISPQ = '0')) ");
			sbSql.append("OR (((T.ISKYPF = '1' AND T.KYPF IS NOT NULL AND T.KYPF_SJ IS NOT NULL) AND ((T.KYPF_SJ - T.KYPF) <= 10 )) OR (T.ISKYPF = '0')) ");
			sbSql.append("OR (((T.ISHPJDS = '1' AND T.HPJDS IS NOT NULL AND T.HPJDS_SJ IS NOT NULL) AND ((T.HPJDS_SJ - T.HPJDS) <= 10 )) OR (T.ISHPJDS = '0')) ");
			sbSql.append("OR (((T.ISGCXKZ = '1' AND T.GCXKZ IS NOT NULL AND T.GCXKZ_SJ IS NOT NULL) AND ((T.GCXKZ_SJ - T.GCXKZ) <= 10 )) OR (T.ISGCXKZ = '0')) ");
			sbSql.append("OR (((T.ISSGXK = '1' AND T.SGXK IS NOT NULL AND T.SGXK_SJ IS NOT NULL) AND ((T.SGXK_SJ - T.SGXK) <= 10)) OR (T.ISSGXK = '0')) ");
			sbSql.append("OR (((T.ISCBSJPF = '1' AND T.CBSJPF IS NOT NULL AND T.CBSJPF_SJ IS NOT NULL) AND ((T.CBSJPF_SJ - T.CBSJPF) <= 5)) OR (T.ISCBSJPF = '0')) ");
			sbSql.append("OR (((T.ISCQT = '1' AND T.CQT IS NOT NULL AND T.CQT_SJ IS NOT NULL) AND ((T.CQT_SJ - T.CQT) <= 5)) OR (T.ISCQT = '0')) ");
			sbSql.append("OR (((T.ISPQT = '1' AND T.PQT IS NOT NULL AND T.PQT_SJ IS NOT NULL) AND ((T.PQT_SJ - T.PQT) <= 5)) OR (T.ISPQT = '0')) ");
			sbSql.append("OR (((T.ISSGT = '1' AND T.SGT IS NOT NULL AND T.SGT_SJ IS NOT NULL) AND ((T.SGT_SJ - T.SGT) <= 5)) OR (T.ISSGT = '0')) ");
			sbSql.append("OR (((T.ISTBJ = '1' AND T.TBJ IS NOT NULL AND T.TBJ_SJ IS NOT NULL) AND ((T.TBJ_SJ - T.CS) <= 5)) OR (T.ISTBJ = '0')) ");
			sbSql.append("OR (((T.ISCS = '1' AND T.CS IS NOT NULL AND T.CS_SJ IS NOT NULL) AND ((T.CS_SJ - T.SGXK) <= 5)) OR (T.ISCS = '0')) ");
			sbSql.append("OR (((T.ISJLDW = '1' AND T.JLDW IS NOT NULL AND T.JLDW_SJ IS NOT NULL) AND ((T.JLDW_SJ - T.JLDW) <= 5)) OR (T.ISJLDW = '0')) ");
			sbSql.append("OR (((T.ISSGDW = '1' AND T.SGDW IS NOT NULL AND T.SGDW_SJ IS NOT NULL) AND ((T.SGDW_SJ - T.SGDW) <= 5)) OR (T.ISSGDW = '0')) ");
			sbSql.append("OR (((T.ISJG = '1' AND T.JG IS NOT NULL AND T.JG_SJ IS NOT NULL) AND ((T.JG_SJ - T.JG) <= 5)) OR (T.ISJG = '0')) ");
			sbSql.append("AND T.ND = '"+year+"' AND T.SFYX = '1' AND T.XMBS = '1' OR (T.ISNOBDXM='1' AND T.XMBS!='1')");
     		sql = sbSql.toString();
        ...
        <%
	    	sbSql = new StringBuffer();
	   	    ...
	   	    sbSql.append("FROM GC_JH_SJ T ");
	   	    sbSql.append("WHERE ");
	   	    sbSql.append("(T.KGSJ IS NOT NULL AND T.KGSJ_SJ IS NULL AND (trunc(SYSDATE) >= T.KGSJ)) ");
	   	    sbSql.append("OR (T.WGSJ IS NOT NULL AND T.WGSJ_SJ IS NULL AND (trunc(SYSDATE) >= T.WGSJ )) ");
	   	   	sbSql.append("OR ((T.ISZC = '1' AND T.ZC IS NOT NULL AND T.ZC_SJ IS NULL AND (trunc(SYSDATE) >= T.ZC)) OR (T.ISZC = '0')) ");
	       	sbSql.append("OR ((T.ISPQ = '1' AND T.PQ IS NOT NULL AND T.PQ_SJ IS NULL AND (trunc(SYSDATE) >= T.PQ)) OR (T.ISPQ = '0')) ");
	       	sbSql.append("OR ((T.ISKYPF = '1' AND T.KYPF IS NOT NULL AND T.KYPF_SJ IS NULL AND ((trunc(SYSDATE) > T.KYPF) AND (trunc(SYSDATE) - T.KYPF) > 10 )) OR (T.ISKYPF = '0')) ");
	       	sbSql.append("OR ((T.ISHPJDS = '1' AND T.HPJDS IS NOT NULL AND T.HPJDS_SJ IS NULL AND ((trunc(SYSDATE) > T.HPJDS) AND (trunc(SYSDATE) - T.HPJDS) > 10 )) OR (T.ISHPJDS = '0')) ");
	       	sbSql.append("OR ((T.ISGCXKZ = '1' AND T.GCXKZ IS NOT NULL AND T.GCXKZ_SJ IS NULL AND ((trunc(SYSDATE) > T.GCXKZ) AND (trunc(SYSDATE) - T.GCXKZ) > 10 )) OR (T.ISGCXKZ = '0')) ");
	       	sbSql.append("OR ((T.ISSGXK = '1' AND T.SGXK IS NOT NULL AND T.SGXK_SJ IS NULL AND ((trunc(SYSDATE) > T.SGXK) AND (trunc(SYSDATE) - T.SGXK) > 10 )) OR (T.ISSGXK = '0')) ");
	       	sbSql.append("OR ((T.ISCBSJPF = '1' AND T.CBSJPF IS NOT NULL AND T.CBSJPF_SJ IS NULL AND ((trunc(SYSDATE) > T.CBSJPF) AND (trunc(SYSDATE) - T.CBSJPF) > 5 )) OR (T.ISCBSJPF = '0')) ");
	       	sbSql.append("OR ((T.ISCQT = '1' AND T.CQT IS NOT NULL AND T.CQT_SJ IS NULL AND ((trunc(SYSDATE) > T.CQT) AND (trunc(SYSDATE) - T.CQT) > 5 )) OR (T.ISCQT = '0')) ");
	       	sbSql.append("OR ((T.ISPQT = '1' AND T.PQT IS NOT NULL AND T.PQT_SJ IS NULL AND ((trunc(SYSDATE) > T.PQT) AND (trunc(SYSDATE) - T.PQT) > 5 )) OR (T.ISPQT = '0')) ");
	       	sbSql.append("OR ((T.ISSGT = '1' AND T.SGT IS NOT NULL AND T.SGT_SJ IS NULL AND ((trunc(SYSDATE) > T.SGT) AND (trunc(SYSDATE) - T.SGT) > 5 )) OR (T.ISSGT = '0')) ");
	       	sbSql.append("OR ((T.ISTBJ = '1' AND T.TBJ IS NOT NULL AND T.TBJ_SJ IS NULL AND ((trunc(SYSDATE) > T.TBJ) AND (trunc(SYSDATE) - T.TBJ) > 5 )) OR (T.ISTBJ = '0')) ");
	       	sbSql.append("OR ((T.ISCS = '1' AND T.CS IS NOT NULL AND T.CS_SJ IS NULL AND ((trunc(SYSDATE) > T.CS) AND (trunc(SYSDATE) - T.CS) > 5 )) OR (T.ISCS = '0')) ");
	       	sbSql.append("OR ((T.ISJLDW = '1' AND T.JLDW IS NOT NULL AND T.JLDW_SJ IS NULL AND ((trunc(SYSDATE) > T.JLDW) AND (trunc(SYSDATE) - T.JLDW) > 5 )) OR (T.ISJLDW = '0')) ");
	       	sbSql.append("OR ((T.ISSGDW = '1' AND T.SGDW IS NOT NULL AND T.SGDW_SJ IS NULL AND ((trunc(SYSDATE) > T.SGDW) AND (trunc(SYSDATE) - T.SGDW) > 5 )) OR (T.ISSGDW = '0')) ");
	       	sbSql.append("OR ((T.ISJG = '1' AND T.JG IS NOT NULL AND T.JG_SJ IS NULL AND ((trunc(SYSDATE) > T.JG) AND (trunc(SYSDATE) - T.JG) > 5 )) OR (T.ISJG = '0')) ");
	       	sbSql.append("AND T.ND = '"+year+"' AND T.SFYX = '1' AND T.XMBS = '1' OR (T.ISNOBDXM='1' AND T.XMBS!='1')");
       		sql = sbSql.toString();
...
<br>
      ...
      <tr>
        ...
        <%
     		sbSql = new StringBuffer();
    	    sbSql.append("SELECT a+b+c+d+e+f+g+h+i+j+k+l+m+n+o+p as ZJDS FROM( ");
    	    sbSql.append("SELECT distinct ");
    	    sbSql.append("(SELECT COUNT(GC_JH_SJ_ID)*2 FROM GC_JH_SJ WHERE T.ND = '"+year+"' AND T.SFYX = '1') as a, ");
    	    sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISKYPF = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as b, ");
    	    sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISHPJDS = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as c, ");
    	    sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISGCXKZ = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as d, ");
    	    sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISSGXK = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as e, ");
    	    sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISCBSJPF = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as f, ");
    	    sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISCQT = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as g, ");
    	  	sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISPQT = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as h, ");
	       	sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISSGT = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as i, ");
	       	sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISTBJ = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as j, ");
	       	sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISCS = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as k, ");
	       	sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISJLDW = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as l,");
	       	sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISSGDW = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as m, ");
	       	sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISZC = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as n, ");
	       	sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISPQ = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as o, ");
	       	sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ISJG = '1' AND T.ND = '"+year+"' AND T.SFYX = '1') as p ");
        ...
        <%
    		sbSql = new StringBuffer();
   	        ...
   	        sbSql.append("SELECT distinct ");
   	        sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND KGSJ_SJ IS NOT NULL) as a,");
   	        sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND WGSJ_SJ IS NOT NULL) as q, ");
        ...
        <%
     		sbSql = new StringBuffer();
       		...
       		sbSql.append("SELECT distinct ");
       		sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND KGSJ_SJ IS NULL AND KGSJ IS NOT NULL AND (trunc(SYSDATE) >= KGSJ)) as a,");
       		sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND WGSJ_SJ IS NULL AND WGSJ IS NOT NULL AND (trunc(SYSDATE) >= WGSJ)) as q, ");
        ...
        <%
        				sbSql = new StringBuffer();
       	         		...
       	         		sbSql.append("SELECT distinct ");
       	         		sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND (KGSJ >= KGSJ_SJ)) as a,");
       	         		sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND (WGSJ >= WGSJ_SJ)) as q, ");
        ...
        <%
        				sbSql = new StringBuffer();
       	         		...
       	         		sbSql.append("SELECT distinct ");
       	         		sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND (KGSJ_SJ > KGSJ)) as a,");
       	         		sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND (WGSJ_SJ >WGSJ)) as q, ");
        ...
        <%
        				sbSql = new StringBuffer();
       	         		...
       	         		sbSql.append("SELECT distinct ");
       	         		sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND KGSJ_SJ IS NULL AND (trunc(SYSDATE) > KGSJ)) as a,");
       	         		sbSql.append("(SELECT COUNT(GC_JH_SJ_ID) FROM GC_JH_SJ WHERE ND = '"+year+"' AND SFYX = '1' AND WGSJ_SJ IS NULL AND (trunc(SYSDATE) >WGSJ)) as q, ");
      ...
      <tr>
        ...
        <td><div class="test">
            ...
            <thead>
              <tr class="bzgk-table-tr-Border">
                ...
                <th colspan="5" align="center" valign="middle">项目数量</th>
              </tr>
              <tr>
                <th valign="middle">总数</th>
        ...
        for(int i = 0; i < xmglgsCoount; i++){
        	xmglgsId = xmglgsValue[i][0].toString();
        	xmglgsText = xmglgsValue[i][1].toString();
        	sbSql.append("SELECT DISTINCT '"+xmglgsText+"', ");
        	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1') AND XMGLGS ='"+xmglgsId+"'), ");
        	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1' AND ISZC = '1'AND ZC_SJ IS NOT NULL) AND XMGLGS ='"+xmglgsId+"'), ");
        	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1' AND ISPQ = '1' AND PQ_SJ IS NOT NULL) AND XMGLGS = '"+xmglgsId+"'), ");
        	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1' AND KGSJ_SJ IS NOT NULL) AND XMGLGS ='"+xmglgsId+"'), ");
        	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1' AND WGSJ_SJ IS NOT NULL) AND XMGLGS ='"+xmglgsId+"') ");
        	sbSql.append("FROM GC_TCJH_XMXDK ");
        	sbSql.append(" UNION ALL ");
        }
        sbSql.append("SELECT DISTINCT '其他', ");
    	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1') AND XMGLGS IS NULL), ");
    	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1' AND ISZC = '1'AND ZC_SJ IS NOT NULL) AND XMGLGS IS NULL), ");
    	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1' AND ISPQ = '1' AND PQ_SJ IS NOT NULL) AND XMGLGS IS NULL), ");
    	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1' AND KGSJ_SJ IS NOT NULL) AND XMGLGS IS NULL), ");
    	sbSql.append("(SELECT COUNT(GC_TCJH_XMXDK_ID) FROM GC_TCJH_XMXDK WHERE GC_TCJH_XMXDK_ID IN(SELECT XMID FROM GC_JH_SJ WHERE ND ='"+year+"' AND SFYX = '1' AND WGSJ_SJ IS NOT NULL) AND XMGLGS IS NULL) ");
    	sbSql.append("FROM GC_TCJH_XMXDK ");
        sql = sbSql.toString();
        String[][] xmglgsResult = DBUtil.query(conn, sql);
        %>   
        <%
        	int xmglgsHj[] = new int[6];//项目管理公司合计列数
