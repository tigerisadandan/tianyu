...
System.out.println("notReadSql : " + notReadSql);
for(int i = 0 ; i < notReadQs.getRowCount() ; i++) {
//	System.out.println(i+" notReadSql | " +notReadQs.getString(i+1, "jsbm").toString());
}
...
System.out.println("departmentSql : " + departmentSql);
for(int i = 0 ; i < departQs.getRowCount() ; i++) {
//	System.out.println(i+" | " +departQs.getString(i+1, "jsbm").toString());
}
...
<div class="container-fluid">
	...
	<p id="readedList"  style=" cursor: pointer;" title="点击查看已阅读人员"><b  style="font-size:16px;">已阅读人员（<%=readQs.getRowCount()%>人）</b></p>
		...
		<table class="B-table" width="400px">
			...
			<tr style="font-size:13px;">
				<td width="20%" class="right-border bottom-border"><%=readQs.getString(i+1, 4) %></td>
				<td width="8%" class="right-border bottom-border"><%=readQs.getString(i+1, "jsr") %></td>
				<td width="20%" class="right-border bottom-border"><%=readQs.getString(i+1, 5) %></td>
			</tr>
			<%	
					}
				}
	...
	<div id="unRead" style="display: none">
		...
		<table class="B-table" width="800px">
			...
			//		System.out.println(departQs.getString(i+1, 1) + " | " + departQs.getString(i+1, 2) + " | " + line);
				...
				//		if(readQs.getString(j+1, 4).equals(departQs.getString(i+1, 1))) {
							//	System.out.println(readQs.getString(j+1, 3) + " | " + (readQs.getString(j+1, 2) == null) + " | " + readQs.getString(j+1, 4).equals(departQs.getString(i+1, 1)));
						...
						<%
								for(int k = start; k < end; k++) {
									...
									<td style="width: 100px" class="right-border bottom-border">
										<input id="noReadPsnCheck" type="checkbox"/>&nbsp;&nbsp;<%=notReadQs.getString(cccnt, "jsr") %>
									</td>
									<%
									cccnt++;
								}
