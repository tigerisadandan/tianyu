...
<%
  	...
  	String sql = "select accnname,acconame from TAC_ACCESSORY t where t.accdir = 'FlowApply' and t.accbelongid = "+flaid;
	String accname[][] = DBUtil.query(sql);
	
  	String sqlfle = "select t.accnname, t.acconame, "+
  	       			"(select e.dptname from TAB_DEPARTMENT e,TAB_EMPLOYEE f where e.dptid = f.empdptid and f.empid = d.flerempid)||'/'||d.flehempname as fjmc"+
                ...
                <div class="tab-pane" id="attachedfile" style="height:80%;overflow:hidden;">
					...
					<p style="margin-left:30px;">
						...
						%>
									...
									<img src="old/MailHaveFile.gif" align="absmiddle">&nbsp;
									<a id="AccHrefj<%=i+1%>" style="CURSOR: hand; TEXT-DECORATION: none" href="<%=request.getContextPath() %>/PubWS.do?getOldFjAction&filename=<%=fleaccname[i][0]%>&filepath=Flow&filenametype=<%=fleaccname[i][1]%>" target="_black"><%=fleaccname[i][1]%></a><br>
						<%
								}
								out.print("</p>");
							}
