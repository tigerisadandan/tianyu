...
function initClType(){
	var queryconditionXX = "{\"logic\":\"and\",\"fieldname\":\"YWLZ_UID\",\"operation\":\"=\",\"value\":\""+ywlzid+"\",\"fieldtype\":'',\"fieldformat\":''}";
	var dataXX = "{querycondition: {conditions: [" +queryconditionXX+" ]},pages:{recordsperpage:200, currentpagenum:1, totalpage:0, countrows:0},orders:[{\"fieldname\":\"YWLZ_UID\",\"order\":\"asc\"}]}";
	var data = {
   			msg : dataXX
   		};
	$.ajax({
		url : "${pageContext.request.contextPath}/ywlz/buSpYwlzController?queryClType",
		data : data,
		cache : false,
		async : false,
		dataType : "json",
		...
		success : function(response) {
			...
			var strs= new Array();
			var strs2= new Array();
			strs=b.response.data[0].CL_TYPE.split(",");
			for (i=0;i<strs.length ;i++ ) 
			{ 
				strs2=strs[i].split(":");
				$("#"+strs2[0]+""+strs2[1]).prop('checked','checked');
		    } 
		}
	});
}
		...
		success : function(response) {
			if(response!=""){
				...
				$.each(cls,function(i){
		           ...
		           "<input  disabled=\"disabled\" name=\"clradio"+i+"\" id=\""+this.YWCL_UID+"1\" YWCLID=\""+this.YWCL_UID+"\" type=\"radio\" checked=\"\" value=\"1\"/>完成</input>&nbsp;&nbsp;&nbsp; "+
		           "<input  disabled=\"disabled\" name=\"clradio"+i+"\" id=\""+this.YWCL_UID+"-1\" YWCLID=\""+this.YWCL_UID+"\" type=\"radio\" checked=\"\" value=\"-1\"/>未完成</input>&nbsp;&nbsp;&nbsp;"+
		           "<input  disabled=\"disabled\" name=\"clradio"+i+"\" id=\""+this.YWCL_UID+"0\" YWCLID=\""+this.YWCL_UID+"\" type=\"radio\" checked=\"\" value=\"0\"/>需整改</input>&nbsp;&nbsp;&nbsp;"+
		           "</span>";
...
<div class="container-fluid">
	...
	<div class="row-fluid">
		...
		<h4 class="title">
			<span id="yw_title">实例业务流转>详细信息</span>	
		</h4>
