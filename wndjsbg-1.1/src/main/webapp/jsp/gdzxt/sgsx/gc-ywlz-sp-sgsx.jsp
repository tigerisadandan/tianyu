...
<app:base />
<title>审批业务流转实例审批(建筑工程施工审批页面)</title>
<%
...
//页面初始化
	$(function() {
		...
		//初始化施工119手续 信息 
		var LJR_DATE=new Date();
		var year=LJR_DATE.getFullYear();
		var month=LJR_DATE.getMonth()+1;
		var date=LJR_DATE.getDate(); 
		$("#LJR_DATE").val(year+"-"+month+"-"+date);
	    getLjrCode();
	});
var wj_PROJECTS_UID;
...
function getLjrCode(){
	$.ajax({
		url : "${pageContext.request.contextPath}/ywlz/buSpYwlzBzController?queryYwLzBzLjrCode",
	//	data : data1,
		cache : false,
		async : false,
		dataType : "json",
...
function init(){
	...
	$.ajax({
		...
		ywlz_uid = ywlz_uid;//tempJson.YWLZ_UID;
		wj_PROJECTS_UID=projects_uid;//tempJson.PROJECTS_UID;
		//wj_JS_COMPANY_UID=tempJson.JS_COMPANY_UID;
		var spyw_uid =spywid// tempJson.SPYW_UID;
		//var projects_uid =tempJson.PROJECTS_UID;
		var spywmc;
		if(spyw_uid == "119"){
		   spywmc ="建筑工程施工注册证及安监备案";
		}
		//var spywmc =tempJson.SPYWMC;
		//var company_name=tempJson.COMPANY_NAME;
		//$("#ywlzmcid").html(company_name+"-"+spywmc);
		//列出审批意见信息
		...
		$("#YWLZ_UID_ID_HF").val(spyw_uid);//tempJson.YWLZ_UID
...
window.onunload = function(){ 
		...
		$(function() {		      					
				$("#btnAddYj").click(function() {
					...
					// var chuli_yijian  = $("#CHULI_YIJIAN_ID").val();
					 var querycondition = "";
					 if(spywid=="119"){
						 querycondition = "{\"LJR_PHONE\":\""+$("#LJR_PHONE").val()+"\",\"LJR_DATE\":\""+$("#LJR_DATE").val()+"\",\"LJR_NAME\":\""+$("#LJR_NAME").val()+"\",\"ZS_CODE\":\""+$("#ZS_CODE").val()+"\",\"ZS_NAME\":\""+$("#ZS_NAME").val()+"\",\"LZBZ_UID\":\""+lzbz_uid+"\",\"CHULI_JIEGUO\":\""+chuli_jieguo+"\",\"CHULI_YIJIAN\":\""+chuli_yijian+"\",\"CHAOSHI_YUANYIN\":\""+chaoshi_yuanyin+"\"}";
					 }else{
					 querycondition = "{\"LZBZ_UID\":\""+lzbz_uid+"\",\"CHULI_JIEGUO\":\""+chuli_jieguo+"\",\"CHULI_YIJIAN\":\""+chuli_yijian+"\",\"CHAOSHI_YUANYIN\":\""+chaoshi_yuanyin+"\"}";
					 }
 ...
 /*  function caozuo(obj){//步骤文件 显示/打印
    //  queryScxmmjhsd(obj.BZWJ_UID);
      var url="${pageContext.request.contextPath }"+obj.OPTURL;
      var showHtml= "<a href='#' onclick=\"bzwjadd('"+url+"','"+index+"','"+obj.TMPWJNAME+"')\" ><i class=\"icon-print\" title=\"填表或打印\"></i></a>";
      index++;
      return showHtml;
   }*/
...
/*function bzwjadd(url,obj,filename){ //操作 
//	${pageContext.request.contextPath}/"+this.URL+"/detail:"+this.YWLZ_UID;
    var cbr="";
    var htj="";
    var obj=JSON.parse($("#bzwjDT").getSelectedRowJsonByIndex(obj));
    getLzbzWj(obj);
	window.open(url+":"+wj_PROJECTS_UID+":"+wj_JS_COMPANY_UID+":"+filename+":"+ywlz_uid,"步骤文件打印");
...
<div class="row-fluid">  
	<div class="B-small-from-table-autoConcise">
      <h4 class="title"><span id="ywlzmcid">-建筑工程施工注册证及安监备案</span>
	    ...
	    <table border="1" width="100%" cellpadding="0" cellspacing="0" class="content">
			<tr>
				<td class="yw-title">
					<div class="span6"><h4><span id="cltitle">材料信息</span></h4></div>
					<div class="pull-right">
					    <button id="btnPrint" class="btn" type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;"> 建设工程安监通知书</button>
					</div>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="right-border bottom-border">
					...
					<input class="span12" style="width:93%" id="ZS_NAME" type="text" placeholder="必填" check-type="required" value="安全监督受监通知书" fieldname="ZS_NAME" name="ZS_NAME">
				</td>
			</tr>
			<tr>
			    <th width="15%" class="right-border bottom-border text-right">证书编号</th>
				<td class="right-border bottom-border">
					<input class="span12" style="width:93%" id="ZS_CODE" type="text" placeholder="必填" check-type="required" fieldname="ZS_CODE" name="ZS_CODE">
				</td>
				<th width="15%" class="right-border bottom-border text-right">领件人</th>
				<td class="right-border bottom-border">
					<input class="span12" style="width:93%" id="LJR_NAME" type="text" placeholder="必填" check-type="required" fieldname="LJR_NAME" name="LJR_NAME">
				</td>
			</tr>
			<tr>
			    <th width="15%" class="right-border bottom-border text-right">联系电话</th>
				<td class="right-border bottom-border">
					<input class="span12" style="width:93%" id="LJR_PHONE" type="text" placeholder="必填" check-type="required" fieldname="LJR_PHONE" name="LJR_PHONE">
				</td>
				<th width="15%" class="right-border bottom-border text-right">领件日期</th>
				<td class="right-border bottom-border">
				    <input style="width:85%" id="LJR_DATE" type="text" fieldname="LJR_DATE" name="LJR_DATE" fieldtype="date" fieldformat="YYYY-MM-DD" class="Wdate"  onclick="WdatePicker()">
				</td>
			</tr>
	      </tbody></table>
	      </form>
		...
		<table id="YJ_LIST"  border="1" width="100%" cellpadding="0" cellspacing="0" class="content">
			<tr>
				<td class="yw-title">
					...
					<div class="pull-right">
					    <button id="btnFfzcz" onclick="btnFfzcz()" class="btn" type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;">发放注册证 </button>
					    <button id="btnPrint" class="btn" type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;">发卡 </button>
					    <button id="btnPrint" class="btn" type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;"> 建筑注册登记表 </button>
					    <button id="btnPrint" class="btn" type="button" style="font-family:SimYou,Microsoft YaHei;font-weight:bold;"> 市政注册登记表</button>
					</div>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<table width="100%" class="table-hover table-activeTd B-table" id="bzwjDT" type="single" pageNum="2000" noPage="true">
						<thead>
				          ...
				          <th fieldname="OPTURL" colindex=7 tdalign="center" maxlength="30" >&nbsp;打印&nbsp;</th>
						                	</tr>
						     </thead>
				  	<tbody></tbody>
					</table>  
